...
`;
  ...
  async function pollForPrompt() {
    ...
    try {
      let data = null;
      /* Only poll local bridge — Vercel /api/bridge/prompt returns 404 */
      try {
        const currentAi = detectAiName();
        const pollUrl = `${CONFIG.BRIDGE_SERVER}/v1/bridge/poll?target_ai=${currentAi}`;
        
        const r = await proxyFetch(pollUrl, { method: "GET" });
        if (r && r.ok) {
          const d = await r.json();
          if (d && d.status !== "idle" && d.prompt) {
            console.log(`[KIROV5 BRIDGE] 🎉 PROMPT REÇU POUR INJECTION (${currentAi}) ! Projet: ${d.project_id || 'KIKI'}`);
            data = d;
          }
        }
      } catch (err) {
        if (Math.random() < 0.1) console.warn("[KIROV5 BRIDGE] Erreur polling bridge:", err.message);
      }
