const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-v200-BdodJ84N.js","assets/design-v200-PiB5ZXIy.js","assets/design-v200-N0shPElz.css"])))=>i.map(i=>d[i]);
import{r as o,a as St,j as e,s as _,i as $n,R as ps}from"./design-v200-PiB5ZXIy.js";const ms="modulepreload",gs=function(t){return"/"+t},Nn={},zt=function(n,r,a){let d=Promise.resolve();if(r&&r.length>0){let m=function(f){return Promise.all(f.map(I=>Promise.resolve(I).then(D=>({status:"fulfilled",value:D}),D=>({status:"rejected",reason:D}))))};document.getElementsByTagName("link");const u=document.querySelector("meta[property=csp-nonce]"),S=(u==null?void 0:u.nonce)||(u==null?void 0:u.getAttribute("nonce"));d=m(r.map(f=>{if(f=gs(f),f in Nn)return;Nn[f]=!0;const I=f.endsWith(".css"),D=I?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${f}"]${D}`))return;const w=document.createElement("link");if(w.rel=I?"stylesheet":ms,I||(w.as="script"),w.crossOrigin="",w.href=f,S&&w.setAttribute("nonce",S),document.head.appendChild(w),I)return new Promise((W,E)=>{w.addEventListener("load",W),w.addEventListener("error",()=>E(new Error(`Unable to preload CSS for ${f}`)))})}))}function p(m){const u=new Event("vite:preloadError",{cancelable:!0});if(u.payload=m,window.dispatchEvent(u),!u.defaultPrevented)throw m}return d.then(m=>{for(const u of m||[])u.status==="rejected"&&p(u.reason);return n().catch(p)})},hs=`# undefined
...
- \`/requirements.txt\` & \`/package.json\` (Gestion des dépendances isolées)`,Rs=`> **DIRECTIVE SYSTÈME POUR L'IA (DEEPSEEK / STITCH)**
...
- \`src/shared/components/inputs/CreditCardInput.tsx\``,Es=`> **DIRECTIVE SYSTÈME POUR L'IA (DEEPSEEK / STITCH)**
...
- \`src/features/marketing/components/LeadCaptureForm.tsx\``,Rr=`> **DIRECTIVE SYSTÈME POUR L'IA (DEEPSEEK / STITCH)**
...
- \`src/core/mobile/components/SwipeActionContainer.tsx\``,Er=`> **DIRECTIVE SYSTÈME POUR L'IA (DEEPSEEK / STITCH)**
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const La=o.forwardRef(({color:t="currentColor",size:n=24,strokeWidth:r=2,absoluteStrokeWidth:a,className:d="",children:p,iconNode:m,...u},S)=>o.createElement("svg",{ref:S,...Ma,width:n,height:n,stroke:t,strokeWidth:a?Number(r)*24/Number(n):r,className:rs("lucide",d),...u},[...m.map(([f,I])=>o.createElement(f,I)),...Array.isArray(p)?p:[p]]));/**
 * @license lucide-react v0.378.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
...
Spécifications techniques du pack Guest ${y.name}.`})}},className:"design-prd-btn text-purple-400 text-xs font-bold hover:underline cursor-pointer flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-purple-500/30 transition-all",children:[e.jsx("span",{children:"📖"})," README →"]})]})]},j)})}):e.jsx(Nt,{items:Et.map(y=>{const j=y.id,F=y.name,b=y.icon||is,M=t?t.includes(j):!1,q=(f[j]||dn(j)||"Spécifications techniques du contrat PRD.").replace(/^#+.*$/gm,"").replace(/^>.*$/gm,"").replace(/\[([^\]]+)\]\([^\)]+\)/g,"$1").replace(/[*_~`]/g,"").trim().split(`
...
Câblage en cours...`,widget:"phases"};H(5),S(Q=>[...Q,R]);let B=5;const ne=setInterval(()=>{B++,B>11?clearInterval(ne):H(B)},3e3);if(typeof window<"u"&&window.AndroidBridge&&window.AndroidBridge.showToast&&window.AndroidBridge.showToast("Mode Design-First Activé !"),typeof window<"u"){const Q=localStorage.getItem("tiger_targetAi")||"deepseek";let Z="";h.length>0&&(Z=`
...
\`\`\`
`,G=window.AndroidBridge;if(G&&G.openAIWithPrompt){const ce=Xe(Q);G.openAIWithPrompt(ce,Ne),G.showToast&&G.showToast("HTML injecté. Câblage sur "+Q.toUpperCase()+" !")}else _("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Q,prompt:Ne,auto_submit:!0,project_id:P,phase_num:5})}).then(ce=>console.log("[Bridge] Prompt HTML + Contexte envoyé avec succès, status:",ce.status)).catch(ce=>console.error("[Bridge] Erreur lors de l'envoi du prompt:",ce))}},kt=(s,l=0)=>s?s.type==="directory"?e.jsxs("div",{className:"flex flex-col",children:[e.jsxs("div",{className:"flex items-center gap-2 px-2 py-1 hover:bg-white/5 cursor-pointer text-gray-300 text-sm",style:{paddingLeft:`${l*12+8}px`},children:[e.jsx("span",{className:"text-orange-400",children:"📁"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-dossier truncate font-bold",children:s.name})]}),s.children&&s.children.map(c=>kt(c,l+1))]},s.path):e.jsxs("div",{className:`flex items-center justify-between px-2 py-1 hover:bg-white/10 cursor-pointer text-sm group ${le===s.path?"bg-cyan/20 text-cyan border-l-2 border-cyan":"text-gray-400"}`,style:{paddingLeft:`${l*12+8}px`},onClick:()=>$e(s.path),children:[e.jsxs("div",{className:"flex items-center gap-2 truncate",children:[e.jsx("span",{className:"text-blue-400",children:"📄"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-fichier truncate",children:s.name})]}),e.jsxs("div",{className:"hidden group-hover:flex items-center gap-1",children:[e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-red-500/50 transition-colors",title:"Suture : Réparer ou débugger ce fichier",onClick:c=>{c.stopPropagation(),$e(s.path),setTimeout(()=>Be("suture","Veuillez analyser ce fichier et corriger toutes les erreurs, fautes de frappe ou imports manquants."),100)},children:"🩺"}),e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-cyan/50 transition-colors",title:"Ajouter au Trombone",onClick:c=>{c.stopPropagation(),k(s.path)},children:"📎"})]})]},s.path):null,Be=async(s,l)=>{if(!U){alert("⚠️ Veuillez sélectionner un projet actif dans l'explorateur.");return}const c=localStorage.getItem("tiger_targetAi")||"deepseek";let P="";s==="suture"?P=`🩺 SUTURE CHIRURGICALE — CORRECTION D'ERREUR DÉTECTÉE
...
`),le&&ft&&(P+=`--- Fichier Actif en Édition : ${le} ---
\`\`\`
${ft}
\`\`\`
...
[PACKS PRD ARCHITECTURE SELECTIONNES (${A.length})]
`+A.map(Z=>{const je=Et.find(Ne=>Ne.id===Z);return`• ${je?je.name:Z} (#${Z})`}).join(`
`):"";let B=`
