...
Spécifications techniques du pack Guest ${_.name}.`})}},className:"design-prd-btn text-purple-400 text-xs font-bold hover:underline cursor-pointer flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-purple-500/30 transition-all",children:[e.jsx("span",{children:"📖"})," README →"]})]})]},k)})}):e.jsx(Nt,{items:Rt.map(_=>{const k=_.id,U=_.name,S=_.icon||rs,M=t?t.includes(k):!1,L=(b[k]||dn(k)||"Spécifications techniques du contrat PRD.").replace(/^#+.*$/gm,"").replace(/^>.*$/gm,"").replace(/\[([^\]]+)\]\([^\)]+\)/g,"$1").replace(/[*_~`]/g,"").trim().split(`
...
3. Création du projet et câblage avec Stitch.`}];try{if(typeof window<"u"&&sessionStorage.getItem("deepseek_402")){ct(l),Lt(!1);return}const c=s||localStorage.getItem("tiger_apiKey");if(!c||!c.startsWith("sk-")||c.length<20||typeof window<"u"&&localStorage.getItem("deepseek_solde_epuise")==="true"){ct(l),Lt(!1);return}const T=new AbortController,h=setTimeout(()=>T.abort(),4e3),f=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${c}`},signal:T.signal,body:JSON.stringify({model:"deepseek-chat",messages:[{role:"system",content:"Tu es le fil d'actualités technologiques de l'OS Souverain. Réponds UNIQUEMENT en JSON avec une clé 'news' contenant 3 articles récents au format [{id, tag, title, desc, content}]."},{role:"user",content:"Donne 3 actualités récentes sur l'IA et le développement web."}],response_format:{type:"json_object"}})});if(clearTimeout(h),f.ok){const R=await f.json(),B=JSON.parse(R.choices[0].message.content);B.news&&Array.isArray(B.news)&&B.news.length>0?ct(B.news):ct(l)}else f.status===402&&typeof window<"u"&&(sessionStorage.setItem("deepseek_402","true"),localStorage.setItem("deepseek_solde_epuise","true")),ct(l)}catch(c){console.warn("[Actualités] Erreur ou Timeout API DeepSeek, chargement fallback:",c),ct(l)}finally{Lt(!1)}},[]);o.useEffect(()=>{je()},[je]);const[Ge,Ce]=o.useState(!1),[Ee,tt]=o.useState(()=>localStorage.getItem("tiger_active_theme")||"fold"),[Ue,nt]=o.useState([]),[At,Ut]=o.useState("");o.useEffect(()=>{const s=localStorage.getItem("tiger_saved_themes");if(s)try{let l=JSON.parse(s);if(!Array.isArray(l))throw new Error("Not an array");const c="linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)";l=l.map(T=>T.name==="fold"?{...T,colors:{...T.colors,"bg-app":c}}:T),nt(l)}catch{nt([])}else{const l={name:"fold",colors:{"icon-settings":"linear-gradient(135deg, #c87058, #934a36)","icon-ai":"linear-gradient(135deg, #9d508e, #622e5a)","icon-projects":"linear-gradient(135deg, #d38b5d, #a26038)","icon-packs":"linear-gradient(135deg, #445499, #252e66)","icon-news":"linear-gradient(135deg, #389eb2, #1f6475)","bg-app":"linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)"}};nt([l]),localStorage.setItem("tiger_saved_themes",JSON.stringify([l]))}},[]),o.useEffect(()=>{var s;if(localStorage.setItem("tiger_active_theme",Ee),Ee!=="random"){const l=((s=Ue.find(c=>c.name===Ee))==null?void 0:s.colors["bg-app"])||"linear-gradient(135deg, #1e1b4b 0%, #0f172a 50%, #000000 100%)";document.body.style.background=l,document.body.style.backgroundAttachment="fixed"}else document.body.style.background=""},[Ee,Ue]);const Ve=()=>{if(!At.trim())return alert("Nom invalide");if(Ue.length>=10)return alert("Limite atteinte ! Vous ne pouvez sauvegarder que 10 thèmes maximum. Veuillez en supprimer un pour continuer.");const s={"icon-settings":a("icon-settings",.8),"icon-ai":a("icon-ai",.8),"icon-projects":a("icon-projects",.8),"icon-packs":a("icon-packs",.8),"icon-news":a("icon-news",.8),"bg-app":a("bg-app",.9)},l={name:At.trim(),colors:s},c=[...Ue,l];nt(c),localStorage.setItem("tiger_saved_themes",JSON.stringify(c)),tt(l.name),Ut("")},tn=(s,l)=>{l.stopPropagation();const c=Ue.filter(T=>T.name!==s);nt(c),localStorage.setItem("tiger_saved_themes",JSON.stringify(c)),Ee===s&&tt("random")},nn=s=>{K(l=>l.includes(s)?l.filter(c=>c!==s):[...l,s])},[Re,Pt]=o.useState(()=>{if(typeof window<"u"){const s=sessionStorage.getItem("tiger_selectedStartPhase");return s?Number(s):0}return 0});o.useEffect(()=>{typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase",String(Re))},[Re]);const[ft,Ot]=o.useState(!0),[Oe,dt]=o.useState(!0),[O,Ie]=o.useState(null),[Ft,Tt]=o.useState(null),[we,kt]=o.useState(null),[ce,$e]=o.useState(null),[bt,We]=o.useState(""),[ut,st]=o.useState([]);o.useEffect(()=>{C("http://localhost:5006/api/projects").then(s=>s?s.json():null).then(s=>{if(s&&s.success&&Array.isArray(s.projects))re(s.projects.map(l=>({name:l,desc:"Projet local",bg:""})));else{const l=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:l?{Authorization:`Bearer ${l}`}:{}}).then(c=>c?c.json():null).then(c=>{c&&c.projects&&re(c.projects.map(T=>({name:T.project_id||T.title,desc:"SaaS Cloud",bg:""})))}).catch(()=>{})}}).catch(()=>{})},[]);const[rt,Pe]=o.useState(null),[vt,Ae]=o.useState(""),[sn,Gt]=o.useState(!1),[Fe,i]=o.useState(!1),g=o.useRef(null);o.useEffect(()=>{if(g.current=null,O==="v0-guest"||O==="V0-Guest")i(!0),Pe("http://localhost:3007"),Ae("http://localhost:3007"),g.current="http://localhost:3007";else if(O){i(!0);const l=we&&JSON.stringify(we).includes("next.config")?"http://localhost:3000":"http://127.0.0.1:5175";Pe(l),Ae(l),g.current=l}else i(!1),Pe(null)},[O,we]),o.useEffect(()=>{const s=l=>{var c,T,h,f;if(((c=l.data)==null?void 0:c.type)==="CHANGE_PREVIEW_URL"){const R=l.data.route,B=R==="/"?"":`#${R}`;Pe(ne=>{const Q=ne?ne.split("#")[0]:"http://127.0.0.1:5175/";return`${Q.endsWith("/")?Q:`${Q}/`}${B}`})}if(((T=l.data)==null?void 0:T.type)==="DESIGN_ELEMENT_CLICKED"||((h=l.data)==null?void 0:h.type)==="DESIGN_ELEMENT_DRAGGED"||((f=l.data)==null?void 0:f.type)==="DESIGN_ELEMENT_RESIZED"){const R=document.querySelector('iframe[title="Studio Admin Design"]');R&&R.contentWindow&&R.contentWindow.postMessage(l.data,"*")}};return window.addEventListener("message",s),()=>window.removeEventListener("message",s)},[]),o.useEffect(()=>{O&&C(`http://localhost:5006/api/fs/tree?project=${O}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&kt(s.tree)}).catch(()=>{})},[O]),o.useEffect(()=>{O&&ce&&C(`http://localhost:5006/api/fs/read?project=${O}&file=${encodeURIComponent(ce)}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&We(s.content)}).catch(()=>{})},[O,ce]);const v=async s=>{if(s===void 0||!O||!ce)return;if(We(s),Mt.isNativePlatform()||!!window.AndroidBridge)try{await Vn.writeFile({path:`v0-moteur-mobile/projetv0/${O}/${ce}`,data:s,directory:pn.Documents,encoding:Cn.UTF8,recursive:!0})}catch(c){console.error("Erreur de sauvegarde mobile:",c)}else C("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:O,file:ce,content:s})}).catch(()=>{})},E=s=>{O&&C(`http://localhost:5006/api/fs/read?project=${O}&file=${encodeURIComponent(s)}`).then(l=>l?l.json():null).then(l=>{l&&l.success&&st(c=>c.find(T=>T.path===s)?c:[...c,{path:s,content:l.content}])}).catch(()=>{})};o.useEffect(()=>{(async()=>{const l=["bg-gradient-to-br from-[#bf6969]/80 to-[#c27042]/90 backdrop-blur-md","bg-gradient-to-br from-[#a387b9]/80 to-[#aa6b73]/90 backdrop-blur-md","bg-gradient-to-br from-[#e4a37f]/80 to-[#bf6969]/90 backdrop-blur-md","bg-gradient-to-br from-[#aa6b73]/80 to-[#c27042]/90 backdrop-blur-md"];if(Mt.isNativePlatform()||!!window.AndroidBridge)try{const h=(await Vn.readdir({path:"v0-moteur-mobile/projetv0",directory:pn.Documents})).files.map(f=>f.name||f.toString());re(h.map((f,R)=>({name:f,desc:"Mémoire Téléphone",bg:l[R%l.length]})))}catch{console.log("Aucun projet mobile trouvé ou dossier inexistant.")}else C("http://localhost:5006/api/projects-v2").then(T=>T?T.json():null).then(T=>{if(T&&T.success&&T.projects)re(T.projects.map((h,f)=>({name:h.name,desc:"Environnement Local",bg:l[f%l.length],installed:h.installed})));else{const h=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:h?{Authorization:`Bearer ${h}`}:{}}).then(f=>f?f.json():null).then(f=>{f&&f.projects&&re(f.projects.map((R,B)=>({name:R.project_id||R.title,desc:"SaaS Cloud (Neon DB)",bg:l[B%l.length]})))}).catch(()=>{})}}).catch(()=>{})})()},[]),o.useEffect(()=>{L(!0)},[]);const[se,w]=o.useState(!0);o.useEffect(()=>{const s=()=>w(!0);window.addEventListener("open-mouchard",s);const l=c=>{if(c.data&&c.data.type==="TIGER_CAPTURE"){const T=c.data.data;if(T){let h="index.html";(T.includes("import React")||T.includes("export default"))&&(h="App.tsx"),T.includes("tailwindcss")&&(h="index.css"),$e(h),v(T),kt(f=>{var ne;const R={name:h,path:h,type:"file"};return f?((ne=f.children)==null?void 0:ne.find(Q=>Q.name===h))?f:{...f,children:[...f.children||[],R]}:{name:"Mobile_Storage",type:"directory",children:[R]}})}}};return window.addEventListener("message",l),()=>{window.removeEventListener("open-mouchard",s),window.removeEventListener("message",l)}},[O,ce]),o.useEffect(()=>{if(!j)return;const s=()=>{C("http://localhost:5006/api/bridge/queue").then(c=>c?c.json():null).then(c=>{c&&c.success&&(setBridgeQueueData({current:c.current||{},queue:c.queue||[]}),(c.queue&&c.queue.length>0||c.current&&Object.keys(c.current).length>0)&&setIsPipelineRunning(!0))}).catch(()=>{}),C("http://localhost:5006/api/bridge/logs").then(c=>c?c.json():null).then(c=>{if(c&&c.success&&c.logs){ge(c.logs);const T=[...c.logs].reverse().find(h=>h.includes("URL_PREVIEW="));if(T){const h=T.split("URL_PREVIEW=")[1];h!==g.current&&(g.current=h,Pe(h),Ae(h))}}}).catch(()=>{}),C("http://localhost:5006/api/bridge/autonomous-status").then(c=>c?c.json():null).then(c=>{if(c&&c.success&&(c.data||c.status)){const h=(c.data||c.status).state;h==="diagnosing"||h==="repair_required"||h==="error"?S("error"):["repair_planned","patching_staging","validating","browser_verifying","repairing","working"].includes(h)||["starting_server","launch_requested","restarting"].includes(h)?S("working"):h==="ready"?S("clean"):O&&C(`http://localhost:5006/api/suture/status?project=${O}`).then(f=>f?f.json():null).then(f=>{f&&f.success&&f.status&&S(f.status)}).catch(()=>{})}}).catch(()=>{})};s();const l=setInterval(s,2e3);return()=>clearInterval(l)},[j,O]);const pe=s=>{const l=typeof s=="string"?s:b;if(!l.trim())return;const c={id:Date.now().toString(),role:"user",content:l};y(h=>[...h,c]);const T=l.toLowerCase();setTimeout(()=>{let h={id:(Date.now()+1).toString(),role:"assistant",content:""};const f=T.normalize("NFD").replace(/[\u0300-\u036f]/g,"");if(f==="mes projets"||f.includes("liste des projet")||f==="projet"||f==="projets")h.content="Voici la liste de vos projets récents :",h.widget="projects";else if((f.includes("stitch")||f.includes("deepseek")||f.includes("design")||f.includes("logique"))&&(f.includes("modifi")||f.includes("ajoute")||f.includes("change")||f.includes("mise a jour")||f.includes("evolue")||f.includes("reprends")||f.includes("continue"))){const R=l.match(/\[(.*?)\]/),B=R?R[1]:null,ne=f.includes("deepseek")?"deepseek":"stitch";h.content=`🔄 MODE ÉVOLUTION ACTIVÉ (${ne.toUpperCase()}) 🔄
...
Les intelligences artificielles sont informées et en attente. Une fois le design terminé, glissez l'HTML ici pour lancer le câblage final en phase 5.`,h.widget="phases",H(1);const R=O||(me.trim()?me.trim().replace(/[^a-zA-Z0-9_-]/g,"_"):"Projet_"+l.substring(0,15).replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4));if(Ie(R),typeof window<"u"&&localStorage.setItem("tiger_lastGeneratedProject",R),typeof window<"u"){const Q=localStorage.getItem("tiger_targetAi")||"deepseek",Z=localStorage.getItem("tiger_targetUiAi")||"stitch",Te=(G,de=!1)=>Qe(G,de),Ne=window.AndroidBridge;if(Ne&&Ne.openAIWithPrompt)Ne.openAIWithPrompt(Te(Z,!0),"Génère l'interface UI/UX complète et moderne pour ce projet : "+l),Ne.showToast&&Ne.showToast("Stitch s'ouvre. Générez le HTML, puis utilisez le Trombone.");else{const G=P&&P.length>0?`

[PACKS PRD ARCHITECTURE SELECTIONNES (${P.length})]
`+P.map(Se=>{const ie=Rt.find(xe=>xe.id===Se);return`• ${ie?ie.name:Se} (#${Se})`}).join(`
`):"",de="Génère l'interface UI/UX complète et moderne pour ce projet : "+l+G;try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(de)}catch{}const at=()=>C("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:de,auto_submit:!0,project_id:R,phase_num:1,packs:P})});Re!==200&&(Oe?window.open(Te(Z,!0),"kirov5_ai_target"):window.open(Te(Z,!0),"_blank"),at().then(()=>{console.log("Prompt UI envoyé avec succès au Bridge pour Stitch !")}).catch(Se=>console.error("Erreur sendUiPrompt",Se))),setTimeout(()=>{const Se=P&&P.length>0?l+G:"L'interface UI/UX est actuellement en cours de génération. Prépare la structure backend et les états React pour un projet complexe : "+l+". Reste en attente, je te fournirai le fichier HTML pour le câblage final.";try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(Se)}catch{}window.open(Te(Q),"_blank"),C("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Q,prompt:Se,auto_submit:!0,project_id:R,phase_num:1,packs:P})}).catch(ie=>console.log("Erreur Bridge Logic Prompt:",ie)),P&&P.length>0?C("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Q,user_prompt:Se,packs:P,target_project:R})}).then(()=>{console.log("Méga-Prompt Trombone envoyé !")}).catch(ie=>{console.log("Erreur Trombone Bridge:",ie)}):C("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Q,prompt:Se,auto_submit:!0,project_id:R,phase_num:1})}).then(()=>{console.log("Prompt Logique envoyé au Bridge")}).catch(()=>{})},800),console.log("Les IA ont été ouvertes. Les prompts ont été envoyés au Bridge local pour injection via l'extension.")}}H(1);let B=1;const ne=setInterval(()=>{B++,B>11?clearInterval(ne):H(B)},1500)}else if(f.includes("youtube")||f.includes("video"))h.content="Voici les résultats YouTube pour votre recherche :",h.widget="youtube";else if(f.includes("actualite")||f.includes("ia")||f.includes("news")||f.includes("recherche")){const R=localStorage.getItem("tiger_apiKey");h.content="Actualités & Dernières Innovations IA :",h.widget="news",je(R||void 0)}else if(f.includes("parametre")||f.includes("reglage")||f.includes("configuration")||f.includes("setting"))h.content="Ouverture du panneau de configuration système :",h.widget="settings";else if(f.includes("[plugin:vite:")||f.includes("missing semicolon")||f.includes("syntaxerror")||f.includes("typescriptparsermixin")||f.includes("babel/parser")||f.includes("error")&&f.length>80){const R=O||"Projet_blog_8831";h.content=`🚨 Trace d'erreur détectée !

Lancement immédiat de l'Auto-Suture IA pour le projet "${R}" avec la stack trace fournie... 🩺`,Be("suture",l)}else if(h.content="Traitement de votre demande via Tiger IA...",typeof window<"u"){const R=window.AndroidBridge;R&&R.openAIWithPrompt?R.openAIWithPrompt("https://chat.deepseek.com/",l):window.open("https://chat.deepseek.com/","_blank")}y(R=>[...R,h])},600),x("")},ve=async s=>{var Ne;let l=Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s],c=null;const T=l.filter(G=>G.name.endsWith(".zip"));let h=[];if(T.length>0){y(G=>[...G,{id:Date.now().toString()+"_zip",role:"user",content:"📦 Extraction de l'archive ZIP en cours..."}]);try{const G=(await Ht(async()=>{const{default:de}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:de}},[])).default;for(const de of T){c=de;const Se=await new G().loadAsync(de);for(const[ie,xe]of Object.entries(Se.files))if(!xe.dir&&!ie.includes("__MACOSX")&&!((Ne=ie.split("/").pop())!=null&&Ne.startsWith("."))){const ke=await xe.async("blob"),Xe=new File([ke],ie.split("/").pop()||ie,{type:ke.type});h.push(Xe)}}}catch(G){console.error("Erreur décompression ZIP:",G),alert("Système Kirov5 : Impossible de lire l'archive ZIP.")}}l=[...l.filter(G=>!G.name.endsWith(".zip")),...h];const f=l.filter(G=>G.name.endsWith(".html")),R=l.filter(G=>!G.name.endsWith(".html")&&(G.name.endsWith(".png")||G.name.endsWith(".jpg")||G.name.endsWith(".jpeg")||G.name.endsWith(".md")||G.name.endsWith(".json")||G.name.endsWith(".txt")));if(f.length===0&&R.length===0){alert("Système Kirov5 : Format non supporté. Veuillez déposer au moins un .html, .md, .png, ou un .zip.");return}let B=[...ut];for(const G of R){const de=G.name.endsWith(".png")||G.name.endsWith(".jpg")||G.name.endsWith(".jpeg"),at=await new Promise(ie=>{const xe=new FileReader;xe.onload=ke=>{var Xe;return ie((Xe=ke.target)==null?void 0:Xe.result)},de?xe.readAsDataURL(G):xe.readAsText(G)}),Se={path:G.name,content:de?`[Image jointe visuellement : ${G.name} (Binaire ignoré pour économie de tokens)]`:at};B.some(ie=>ie.path===G.name)||B.push(Se),y(ie=>[...ie,{id:Date.now().toString()+"_"+Math.random(),role:"user",content:`📎 Fichier de contexte ajouté au Trombone : ${G.name}`}])}if(f.length>0){const G=f[0],de=await new Promise(ie=>{const xe=new FileReader;xe.onload=ke=>{var Xe;return ie((Xe=ke.target)==null?void 0:Xe.result)},xe.readAsText(G)}),at={path:G.name,content:de};B.some(ie=>ie.path===G.name)||B.push(at);const Se={id:Date.now().toString()+"_main",role:"user",content:`📁 Fichier de design déposé : ${G.name}
Analyse de la structure UI et assemblage du contexte en cours...`};y(ie=>[...ie,Se])}const ne=(Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s]).filter(G=>G.name.endsWith(".zip"));for(const G of ne)B.some(de=>de.path===G.name)||B.push({path:G.name,content:"ZIP_ARCHIVE_DUMMY"});st(B);const Q=O||me||"Projet_Stitch",Z=l.map(G=>G.name).join(", ");console.log(`[TROMBONE] Fichiers déposés pour ${Q}: ${Z}`);const Te=f.length;if(Te>3){const G=localStorage.getItem("tiger_targetAi")||"deepseek",de=c?"Projet_ZIP_"+c.name.replace(/[^a-zA-Z0-9]/g,"_").replace(".zip","")+"_"+Date.now().toString().slice(-4):"Projet_BATCH_"+Date.now().toString().slice(-4),at=O||de;O||Ie(at),y(ie=>[...ie,{id:Date.now().toString()+"_batch",role:"assistant",content:`🤖 KIROV5 MULTI-BATCH DÉTECTÉ !

Hermes a détecté ${Te} pages HTML dans votre ZIP.
Découpage automatique en lots de 3 fichiers en cours...
Démarrage de l'automatisation séquentielle via DeepSeek !`,widget:"phases"}]),H(1);let Se="";if(c)Se=await new Promise(ie=>{const xe=new FileReader;xe.onload=ke=>{var Xe;return ie(((Xe=ke.target)==null?void 0:Xe.result).split(",")[1])},xe.readAsDataURL(c)});else{const ie=(await Ht(async()=>{const{default:ke}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:ke}},[])).default,xe=new ie;for(const ke of l)xe.file(ke.name,ke);Se=await xe.generateAsync({type:"base64"})}C("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({zip_base64:Se,zip_mode:!0,target_ai:G,target_project:at,user_prompt:me.trim()?`${me} - ${ot}`:`Application multi-pages issue de ${(c==null?void 0:c.name)||"fichiers_locaux"}`})}).then(async ie=>{const xe=await ie.json();xe.success?(y(ke=>[...ke,{id:Date.now().toString()+"_hermes",role:"assistant",content:`✅ ${xe.message}

📡 Lot 1/${xe.total_batches} transmis à DeepSeek. Les lots suivants s'enchaîneront automatiquement !
Fichiers détectés : ${(xe.files_detected||[]).join(", ")}`}]),window.open(Qe(G),"_blank")):alert("Erreur Hermes : "+(xe.error||"Inconnue"))}).catch(ie=>{alert(`Erreur de connexion au Moteur Local : ${ie.message}`)});return}if(f.length>0){if(document.getElementById("creation-mode-container"))return;setTimeout(()=>{Je(B)},1200)}},Je=async(s=ut)=>{Ze(!1);const l=s.find(Q=>Q.path.endsWith(".html"));if(!l){alert("Aucun fichier HTML trouvé dans le Trombone !");return}const c=me.trim()?"Projet_"+me.replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4):"Design_"+l.path.replace(".html","").replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4),T=O||c;if(!O){Ie(T);try{await C("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:T,file:"README.md",content:`# ${me||T}
...
[Fichier: ${de.path}]
\`\`\`
${de.content.substring(0,3e3)}
\`\`\`
`}));let Te="";(me.trim()||qe.trim()||ot.trim())&&(Te=`
--- MÉTADONNÉES DU PROJET ---
Nom : ${me||"Non spécifié"}
Stack / Structure : ${qe||"Non spécifiée"}
Description & Objectifs : ${ot||"Non spécifiés"}
-----------------------------
...
\`\`\`
`,G=window.AndroidBridge;if(G&&G.openAIWithPrompt){const de=Qe(Q);G.openAIWithPrompt(de,Ne),G.showToast&&G.showToast("HTML injecté. Câblage sur "+Q.toUpperCase()+" !")}else C("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Q,prompt:Ne,auto_submit:!0,project_id:T,phase_num:5})}).then(de=>console.log("[Bridge] Prompt HTML + Contexte envoyé avec succès, status:",de.status)).catch(de=>console.error("[Bridge] Erreur lors de l'envoi du prompt:",de))}},jt=(s,l=0)=>s?s.type==="directory"?e.jsxs("div",{className:"flex flex-col",children:[e.jsxs("div",{className:"flex items-center gap-2 px-2 py-1 hover:bg-white/5 cursor-pointer text-gray-300 text-sm",style:{paddingLeft:`${l*12+8}px`},children:[e.jsx("span",{className:"text-orange-400",children:"📁"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-dossier truncate font-bold",children:s.name})]}),s.children&&s.children.map(c=>jt(c,l+1))]},s.path):e.jsxs("div",{className:`flex items-center justify-between px-2 py-1 hover:bg-white/10 cursor-pointer text-sm group ${ce===s.path?"bg-cyan/20 text-cyan border-l-2 border-cyan":"text-gray-400"}`,style:{paddingLeft:`${l*12+8}px`},onClick:()=>$e(s.path),children:[e.jsxs("div",{className:"flex items-center gap-2 truncate",children:[e.jsx("span",{className:"text-blue-400",children:"📄"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-fichier truncate",children:s.name})]}),e.jsxs("div",{className:"hidden group-hover:flex items-center gap-1",children:[e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-red-500/50 transition-colors",title:"Suture : Réparer ou débugger ce fichier",onClick:c=>{c.stopPropagation(),$e(s.path),setTimeout(()=>Be("suture","Veuillez analyser ce fichier et corriger toutes les erreurs, fautes de frappe ou imports manquants."),100)},children:"🩺"}),e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-cyan/50 transition-colors",title:"Ajouter au Trombone",onClick:c=>{c.stopPropagation(),E(s.path)},children:"📎"})]})]},s.path):null,Be=async(s,l)=>{if(!O){alert("⚠️ Veuillez sélectionner un projet actif dans l'explorateur.");return}const c=localStorage.getItem("tiger_targetAi")||"deepseek";let T="";s==="suture"?T=`🩺 SUTURE CHIRURGICALE — CORRECTION D'ERREUR DÉTECTÉE
...
`),ce&&bt&&(T+=`--- Fichier Actif en Édition : ${ce} ---
\`\`\`
${bt}
\`\`\`

`),ut.length>0&&(T+=`--- Fichiers de contexte du Trombone ---

`,ut.forEach(h=>{T+=`--- ${h.path} ---
\`\`\`
${h.content}
\`\`\`
...
`:qe.includes("Canvas 2D")?B+=`⚠️ CONSTRUCTEUR : Ce projet DOIT IMPÉRATIVEMENT être un jeu Web 2D généré en **React + Vite + TypeScript (TSX) + Canvas 2D**.
