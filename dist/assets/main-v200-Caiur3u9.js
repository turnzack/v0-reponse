...
L'orchestrateur prend le relais pour générer le Backend et intégrer les composants.`:"Mode Cloud SaaS : Bridge non disponible.")}).catch(T=>{console.error(T),alert("Erreur: Le moteur n'a pas pu être contacté.")});return}typeof window<"u"&&(window.open("https://stitch.withgoogle.com/","_blank"),window.dispatchEvent(new CustomEvent("open-mouchard")));const Q=window.KIROV_SELECTED_PACKS&&window.KIROV_SELECTED_PACKS.length>0?window.KIROV_SELECTED_PACKS:F&&F.length>0?F:[];if(Number(y)===0){I("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_project:b,target_ai:"stitch",start_index:1,zip_mode:!0,start_phase:0,auto_pilot:$,instructions:Gt||"Génère l'application avec ces packs et directives.",packs:Q})}).then(T=>T?T.json():null).then(T=>{var tt;const ue=(T==null?void 0:T.prompt)||((tt=T==null?void 0:T.data)==null?void 0:tt.prompt)||"Génération UI/UX Stitch complète.",Ae=Q.length>0?Q.join(", "):"standard";if(C(2),typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase","2"),typeof document<"u"){const Pe=document.getElementById("tiger-mega-prompt-overlay");Pe&&Pe.remove();const Ke=document.createElement("div");Ke.id="tiger-mega-prompt-overlay",Ke.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);z-index:999999;display:flex;align-items:center;justify-content:center;padding:16px;font-family:sans-serif;",Ke.innerHTML=`
                                  ...
                                  <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:12px;">
                                    <div>
                                      ...
                                      <p style="margin:4px 0 0 0;font-size:11px;color:#94a3b8;">
                                        Projet : <strong style="color:#e2e8f0;">${b}</strong> | Pack PRD : <strong style="color:#a855f7;">${Ae}</strong>
                                      </p>
                                    </div>
                              ...
                              `,document.body.appendChild(Ke);const mt=document.getElementById("tiger-prompt-area"),Re=document.getElementById("btn-copy-prompt"),gt=document.getElementById("btn-close-prompt-overlay"),Ne=document.getElementById("btn-open-stitch"),s=()=>{mt&&(mt.select(),mt.setSelectionRange(0,99999),document.execCommand("copy")),navigator.clipboard&&navigator.clipboard.writeText(ue).catch(()=>{}),C(2),Re&&(Re.innerHTML="✅ COPIÉ DANS LE PRESSE-PAPIER !",Re.style.background="linear-gradient(135deg,#10b981,#059669)",setTimeout(()=>{Re&&(Re.innerHTML="📋 COPIER LE MÉGA-PROMPT",Re.style.background="linear-gradient(135deg,#06b6d4,#2563eb)")},3e3))};Re&&(Re.onclick=()=>{s(),C(2)}),gt&&(gt.onclick=()=>{Ke.remove(),C(2)}),Ne&&(Ne.onclick=()=>{window.open("https://stitch.withgoogle.com/","_blank"),C(2)}),s(),C(2)}}).catch(T=>{console.error(T),alert("Erreur de communication avec le serveur Kirov5.")});return}I("/api/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:"stitch",user_prompt:Gt||"Génère l'application avec ces packs et directives.",packs:Q,target_project:b,phase_num:1})}).then(T=>T?T.json():null).then(T=>{var Ae;const ue=(T==null?void 0:T.prompt)||((Ae=T==null?void 0:T.data)==null?void 0:Ae.prompt)||"Génération UI/UX Stitch.";typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText(ue).catch(()=>{}),alert(`🚀 PHASE 1 (STITCH UI/UX) INITIALISÉE !
...
Spécifications techniques du pack Guest ${y.name}.`})}},className:"design-prd-btn text-purple-400 text-xs font-bold hover:underline cursor-pointer flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-purple-500/30 transition-all",children:[e.jsx("span",{children:"📖"})," README →"]})]})]},C)})}):e.jsx(Ut,{items:Lt.map(y=>{const C=y.id,F=y.name,S=y.icon||rs,q=t?t.includes(C):!1,G=(v[C]||dn(C)||"Spécifications techniques du contrat PRD.").replace(/^#+.*$/gm,"").replace(/^>.*$/gm,"").replace(/\[([^\]]+)\]\([^\)]+\)/g,"$1").replace(/[*_~`]/g,"").trim().split(`
`).map(ee=>ee.trim()).filter(ee=>ee.length>20),X=G.length>0?G[0]:"Spécifications techniques du contrat PRD.",B=X.length>180?X.substring(0,180)+"...":X;return e.jsxs("div",{className:`design-carte-carrousel design-prd-carte rounded-2xl p-5 border backdrop-blur-md flex flex-col transition-all shadow-lg relative group ${q?"border-cyan shadow-[0_0_25px_rgba(8,179,201,0.4)]":"border-white/10 hover:border-cyan/50"}`,style:{background:r?o("prd-card-"+C,.7):"rgba(0,0,0,0.7)"},children:[e.jsxs("div",{className:"flex justify-between items-center mb-3",children:[e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx("div",{className:`w-9 h-9 rounded-xl flex items-center justify-center font-bold ${q?"bg-cyan text-black shadow-md":y.color||"bg-cyan/20 text-cyan"}`,children:e.jsx(S,{size:20})}),e.jsx("span",{className:"px-2 py-0.5 bg-cyan/20 text-cyan text-[10px] font-bold rounded-md uppercase tracking-wider",children:"💎 PRD"})]}),n&&e.jsx("button",{onClick:ee=>{ee.stopPropagation(),n(C)},className:`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${q?"bg-cyan text-black shadow-md scale-110":"bg-white/10 text-white/60 hover:bg-white/20 hover:text-white"}`,title:q?"Désélectionner ce pack":"Sélectionner ce pack pour l'IA",children:q?"✓":"+"})]}),e.jsx("h3",{className:"design-carte-titre text-base font-bold text-white mb-2 leading-tight",children:F}),e.jsx("p",{className:"design-carte-desc flex-1 opacity-85 leading-relaxed",children:B}),e.jsxs("div",{className:"mt-auto pt-3 border-t border-white/10 flex items-center justify-between",children:[e.jsx("span",{className:"text-[10px] font-mono text-cyan/80",children:"MODULE PRD"}),e.jsxs("button",{onClick:async()=>{var K;let ee=v[C]||dn(C)||"";try{const oe=await I(`http://localhost:5006/api/bridge/read-file?packId=${encodeURIComponent(C)}`);if(oe&&oe.ok){const fe=await oe.json(),U=fe.content||((K=fe.data)==null?void 0:K.content);U&&U.trim().length>5&&(ee=U,f(te=>({...te,[C]:U})))}}catch(oe){console.warn("API Bridge read fallback",oe)}J({packId:C,packName:F,readmeText:ee||`# 💎 PRD PACK : ${F} (${C})

### ⚙️ Spécifications techniques du contrat PRD

- **Identifiant Pack**: ${C}
- **Statut**: Actif & Intégré au Moteur Sovereign`})},className:"design-prd-btn text-cyan text-xs font-bold hover:underline cursor-pointer flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-cyan/30 transition-all hover:bg-cyan/10",children:[e.jsx("span",{children:"📖"})," Lire le README →"]})]})]},C)})})})})]})},Kn=({isClient:t,getCachedGradient:n,setActiveProject:r})=>{const[o,d]=a.useState([]),[p,m]=a.useState(!0),[u,_]=a.useState(!1),[v,f]=a.useState(null),L=(M=!1)=>{M?_(!0):m(!0);const $=["bg-gradient-to-br from-[#bf6969]/80 to-[#c27042]/90 backdrop-blur-md","bg-gradient-to-br from-[#a387b9]/80 to-[#aa6b73]/90 backdrop-blur-md","bg-gradient-to-br from-[#e4a37f]/80 to-[#bf6969]/90 backdrop-blur-md","bg-gradient-to-br from-[#aa6b73]/80 to-[#c27042]/90 backdrop-blur-md"];I("http://localhost:5006/api/projects").then(w=>w?w.json():null).then(w=>{if(w&&w.success&&w.projects)d(w.projects.map((V,J)=>({name:typeof V=="string"?V:V.projectName||V.name||V.projectId||V.project_id||V.title||`Projet_${J}`,desc:"Environnement Local",bg:$[J%$.length],installed:V.installed}))),m(!1),_(!1);else{const V=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:V?{Authorization:`Bearer ${V}`}:{}}).then(J=>J?J.json():null).then(J=>{J&&J.projects&&d(J.projects.map((y,C)=>({name:y.project_id||y.title||y.name||`Projet_${C}`,desc:"SaaS Cloud (Neon DB)",bg:$[C%$.length]}))),m(!1),_(!1)}).catch(()=>{m(!1),_(!1)})}}).catch(()=>{m(!1),_(!1)})};return a.useEffect(()=>{L();const M=setInterval(()=>L(!0),5e3);return()=>clearInterval(M)},[]),p?e.jsx("div",{className:"p-4 text-cyan text-sm italic",children:"Actualisation de la liste des projets..."}):o.length===0?e.jsxs("div",{className:"p-4 flex flex-col items-center gap-3 text-center",children:[e.jsx("div",{className:"text-cyan text-sm italic",children:"Aucun projet trouvé sur votre disque dur."}),e.jsx("button",{onClick:()=>L(),className:"px-4 py-2 rounded-xl bg-cyan/20 border border-cyan/40 text-cyan text-xs font-bold hover:bg-cyan/40 transition-colors",children:"🔄 Rafraîchir la liste"})]}):e.jsx(Ut,{items:[...o.map((M,$)=>{const w=M.name||"AUDIO",V=async()=>{f(w),r(w);try{window.dispatchEvent(new CustomEvent("open-mouchard")),I("http://localhost:5006/api/bridge/manual-pnpm-run",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:w})}).then(async y=>{if(y){const C=await y.json().catch(()=>null);C!=null&&C.previewUrl&&window.open(C.previewUrl,"_blank")}}).catch(()=>{}),await I(`http://localhost:5006/api/projects/${encodeURIComponent(w)}/launch-design`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:w,open_explorer:!1})}).catch(y=>console.error("Erreur de lancement :",y))}catch(y){console.error("Erreur de lancement :",y)}finally{setTimeout(()=>f(null),2e3)}},J=async y=>{if(y.preventDefault(),y.stopPropagation(),window.confirm(`⚠️ SUPPRESSION DEFINITIVE :
Voulez-vous vraiment supprimer le projet "${w}" de l'interface ET de votre disque dur ?`))try{const C=await I("http://localhost:5006/api/projects/remove-project",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:w})});if(!C){d(S=>S.filter(q=>q.name!==w));return}const F=await C.json();F.success?d(S=>S.filter(q=>q.name!==w)):alert(`Erreur lors de la suppression : ${F.message||"Échec server"}`)}catch(C){alert(`Erreur réseau lors de la suppression : ${C.message}`)}};return e.jsxs("div",{className:"design-carte-carrousel rounded-2xl p-5 border border-white/20 shadow-xl flex flex-col items-center text-center relative overflow-hidden group cursor-pointer",style:{background:t?n("proj-"+$,.7):"rgba(0,0,0,0.5)"},onClick:V,children:[e.jsx("button",{onClick:J,className:"z-20 absolute top-3 right-3 w-8 h-8 rounded-full bg-red-600/80 hover:bg-red-500 text-white font-bold text-xs flex items-center justify-center transition-all shadow-lg hover:scale-110 cursor-pointer",title:"Supprimer ce projet du disque dur et de l'interface",children:"🗑️"}),e.jsx("div",{className:"absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-30 mix-blend-overlay pointer-events-none"}),e.jsx("div",{className:"absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors z-0"}),e.jsxs("div",{className:"z-10 relative pointer-events-none w-full pr-8",children:[e.jsx("div",{className:"text-white/70 text-xs font-bold uppercase tracking-widest drop-shadow-md",children:"PROJET"}),e.jsx("h3",{className:"design-carte-titre text-xl font-black text-white break-all drop-shadow-lg leading-tight w-full",children:w})]}),e.jsx("div",{className:"design-carte-desc z-10 relative text-sm text-white/90 font-medium drop-shadow-md pointer-events-none w-full",children:M.desc}),e.jsxs("div",{className:"z-10 flex items-center gap-2 mt-3",children:[e.jsx("button",{onClick:y=>{y.stopPropagation(),v!==w&&(M.installed===!1?(f(w),I("http://localhost:5006/api/bridge/install-dependencies",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:w})}).then(C=>{alert(C?`📦 Installation des dépendances démarrée pour "${w}" !
Suivez la progression dans les logs.`:"Mode Cloud SaaS : Bridge local non connecté."),f(null),d(F=>F.map(S=>S.name===M.name?{...S,installed:!0}:S))}).catch(()=>f(null))):V())},disabled:v===w,className:`font-bold py-2 px-4 rounded-xl text-xs transition-all flex items-center gap-1.5 ${v===w?"bg-cyan-500/40 text-cyan-200 cursor-wait animate-pulse":M.installed===!1?"bg-orange-500/80 hover:bg-orange-600 text-white cursor-pointer":"bg-white/20 hover:bg-white/40 text-white cursor-pointer"}`,title:M.installed===!1?"Installer les dépendances requises pour ce projet":"Lancer la prévisualisation Vite",children:v===w?e.jsxs(e.Fragment,{children:[e.jsx("span",{className:"animate-spin inline-block",children:"⏳"})," ",M.installed===!1?"Installation...":"Lancement..."]}):M.installed===!1?e.jsx(e.Fragment,{children:"📦 INSTALL"}):e.jsx(e.Fragment,{children:"🚀 PREVIEW"})}),M.installed!==!1&&e.jsxs(e.Fragment,{children:[e.jsx("button",{onClick:y=>{y.stopPropagation(),f(w),I("http://localhost:5006/api/bridge/install-dependencies",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:w})}).then(C=>{alert(C?`🔄 Réinstallation des dépendances démarrée pour "${w}" !`:"Mode Cloud SaaS : Bridge local non connecté."),f(null)}).catch(()=>f(null))},className:"bg-orange-500/30 hover:bg-orange-500/60 text-white font-bold py-2 px-3 rounded-xl text-xs transition-colors flex items-center justify-center border border-orange-500/40 cursor-pointer",title:"Forcer la réinstallation des dépendances (pnpm install)",children:"🔄"}),e.jsx("button",{onClick:async y=>{y.stopPropagation();try{f(w);const C=await I("http://localhost:5006/api/bridge/manual-pnpm-run",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:w})});if(f(null),!C){alert("Mode Cloud SaaS : Moteur local non connecté.");return}const F=await C.json();alert(F.message||`🚀 Serveur Vite démarré pour ${w} !`),F.previewUrl&&window.open(F.previewUrl,"_blank")}catch(C){f(null),alert("Erreur de connexion au bridge: "+C.message)}},className:"font-bold py-2 px-3 rounded-xl text-xs bg-purple-500/60 hover:bg-purple-500 text-white transition-all shadow-md cursor-pointer",title:"Lancer le serveur Vite avec pnpm run dev",children:"💻 PNPM"})]}),e.jsx("button",{onClick:J,className:"bg-red-600/30 hover:bg-red-600/60 text-red-200 border border-red-500/40 font-bold py-2 px-3 rounded-xl text-xs transition-colors flex items-center gap-1",title:"Supprimer définitivement du disque dur",children:"🗑️ Supprimer"})]})]},$)})]})};function Mo({user:t,onLogout:n}={}){var Ne;const r=a.useRef({}),o=(s,l=1)=>typeof window>"u"?`linear-gradient(135deg, rgba(17,17,17,${l}), rgba(34,34,34,${l}))`:(r.current[s]||(r.current[s]=Ro(l)),r.current[s]),d=(t==null?void 0:t.email)||(typeof window<"u"?localStorage.getItem("tiger_currentUserEmail"):null),p=!!(t!=null&&t.isSuperAdmin||d&&d.toLowerCase().trim()==="zacktunr@gmail.com");a.useEffect(()=>{t!=null&&t.email&&typeof window<"u"&&localStorage.setItem("tiger_currentUserEmail",t.email)},[t]);const m=()=>{typeof window<"u"&&(localStorage.removeItem("kirov5_jwt_token"),localStorage.removeItem("tiger_currentUserEmail"),localStorage.removeItem("tiger_lastGeneratedProject"),fetch("/api/auth/logout",{method:"POST"}).catch(()=>{})),n?n():typeof window<"u"&&window.location.reload()},[u,_]=a.useState([]),[v,f]=a.useState(""),L=a.useRef(null),M=a.useRef(null),[$,w]=a.useState(!1),[V,J]=a.useState("connexion"),[y,C]=a.useState(!1),[F,S]=a.useState("clean");a.useRef(0);const[q,z]=a.useState(1),[E,G]=a.useState(!1),[X,B]=a.useState(!1),[ee,K]=a.useState(!1);a.useEffect(()=>{let s=0;const l=setInterval(()=>{var c,P;if(typeof window<"u"){const h=(P=(c=window.Capacitor)==null?void 0:c.isNativePlatform)==null?void 0:P.call(c),x=!!window.AndroidBridge;(h||x)&&(B(!0),clearInterval(l))}s++>10&&clearInterval(l)},100);return()=>clearInterval(l)},[]);const[oe,fe]=a.useState([]),[U,te]=a.useState(""),[pe,he]=a.useState(["> Système Kirov5 initialisé. "+new Date().toLocaleTimeString()]),[ye,ie]=a.useState([]),[j,H]=a.useState([]);a.useEffect(()=>{const s=l=>H(l.detail);return window.addEventListener("update_kirov_packs",s),()=>window.removeEventListener("update_kirov_packs",s)},[]),a.useEffect(()=>{window.KIROV_SELECTED_PACKS=j},[j]);const[A,W]=a.useState(!1),[se,_e]=a.useState(!1),[be,Yt]=a.useState([]),[Pt,We]=a.useState(!1),[qe,ce]=a.useState(""),[xt,Gt]=a.useState(""),[en,rt]=a.useState(!1),[me,Tt]=a.useState(""),[Le,En]=a.useState("Vite + React + Tailwind + TS"),[lt,gn]=a.useState(""),[ft,kt]=a.useState("deepseek"),[Ue,tn]=a.useState(""),[nn,Rn]=a.useState(!0),[sn,rn]=a.useState(!1),[ct,bt]=a.useState(null),[it,vt]=a.useState(!1),[Je,Xe]=a.useState("idle"),[jt,Qe]=a.useState([]),[St,Vt]=a.useState(""),[hn,Et]=a.useState(null);a.useEffect(()=>{let s;return Je==="building"&&(s=setInterval(async()=>{try{const l=await I("http://localhost:5006/api/mobile/build-logs");if(l&&l.ok){const c=await l.json();c.logs&&c.logs.length>0&&Qe(c.logs),c.isBuilding===!1&&c.result&&(c.result.success?(Xe("success"),Et(c.result.apkUrl)):Xe("error"),clearInterval(s))}}catch(l){console.warn("Polling APK error",l)}},1e3)),()=>{s&&clearInterval(s)}},[Je]);const[Bt,at]=a.useState([]),[xn,ge]=a.useState(!1),zt=a.useCallback(async s=>{ge(!0);const l=[{id:"deepseek-r1-v0",tag:"⚡ DEEPSEEK R1",title:"DeepSeek-R1 & Moteur Souverain v0.1",desc:"Capacités de raisonnement avancé et de suture pour interfaces React.",content:`DeepSeek-R1 offre des capacités de raisonnement avancé pour l'analyse de contrats PRD et la génération zéro-touch de projets web complets.
...
3. Création du projet et câblage avec Stitch.`}];try{if(typeof window<"u"&&sessionStorage.getItem("deepseek_402")){at(l),ge(!1);return}const c=s||localStorage.getItem("tiger_apiKey");if(!c||!c.startsWith("sk-")||c.length<20||typeof window<"u"&&localStorage.getItem("deepseek_solde_epuise")==="true"){at(l),ge(!1);return}const P=new AbortController,h=setTimeout(()=>P.abort(),4e3),x=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${c}`},signal:P.signal,body:JSON.stringify({model:"deepseek-chat",messages:[{role:"system",content:"Tu es le fil d'actualités technologiques de l'OS Souverain. Réponds UNIQUEMENT en JSON avec une clé 'news' contenant 3 articles récents au format [{id, tag, title, desc, content}]."},{role:"user",content:"Donne 3 actualités récentes sur l'IA et le développement web."}],response_format:{type:"json_object"}})});if(clearTimeout(h),x.ok){const k=await x.json(),D=JSON.parse(k.choices[0].message.content);D.news&&Array.isArray(D.news)&&D.news.length>0?at(D.news):at(l)}else x.status===402&&typeof window<"u"&&(sessionStorage.setItem("deepseek_402","true"),localStorage.setItem("deepseek_solde_epuise","true")),at(l)}catch(c){console.warn("[Actualités] Erreur ou Timeout API DeepSeek, chargement fallback:",c),at(l)}finally{ge(!1)}},[]);a.useEffect(()=>{zt()},[zt]);const[ot,yt]=a.useState(!1),[Oe,Rt]=a.useState(()=>localStorage.getItem("tiger_active_theme")||"fold"),[Ce,Ve]=a.useState([]),[Se,Nt]=a.useState("");a.useEffect(()=>{const s=localStorage.getItem("tiger_saved_themes");if(s)try{let l=JSON.parse(s);if(!Array.isArray(l))throw new Error("Not an array");const c="linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)";l=l.map(P=>P.name==="fold"?{...P,colors:{...P.colors,"bg-app":c}}:P),Ve(l)}catch{Ve([])}else{const l={name:"fold",colors:{"icon-settings":"linear-gradient(135deg, #c87058, #934a36)","icon-ai":"linear-gradient(135deg, #9d508e, #622e5a)","icon-projects":"linear-gradient(135deg, #d38b5d, #a26038)","icon-packs":"linear-gradient(135deg, #445499, #252e66)","icon-news":"linear-gradient(135deg, #389eb2, #1f6475)","bg-app":"linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)"}};Ve([l]),localStorage.setItem("tiger_saved_themes",JSON.stringify([l]))}},[]),a.useEffect(()=>{var s;if(localStorage.setItem("tiger_active_theme",Oe),Oe!=="random"){const l=((s=Ce.find(c=>c.name===Oe))==null?void 0:s.colors["bg-app"])||"linear-gradient(135deg, #1e1b4b 0%, #0f172a 50%, #000000 100%)";document.body.style.background=l,document.body.style.backgroundAttachment="fixed"}else document.body.style.background=""},[Oe,Ce]);const an=()=>{if(!Se.trim())return alert("Nom invalide");if(Ce.length>=10)return alert("Limite atteinte ! Vous ne pouvez sauvegarder que 10 thèmes maximum. Veuillez en supprimer un pour continuer.");const s={"icon-settings":o("icon-settings",.8),"icon-ai":o("icon-ai",.8),"icon-projects":o("icon-projects",.8),"icon-packs":o("icon-packs",.8),"icon-news":o("icon-news",.8),"bg-app":o("bg-app",.9)},l={name:Se.trim(),colors:s},c=[...Ce,l];Ve(c),localStorage.setItem("tiger_saved_themes",JSON.stringify(c)),Rt(l.name),Nt("")},Be=(s,l)=>{l.stopPropagation();const c=Ce.filter(P=>P.name!==s);Ve(c),localStorage.setItem("tiger_saved_themes",JSON.stringify(c)),Oe===s&&Rt("random")},Ht=s=>{H(l=>l.includes(s)?l.filter(c=>c!==s):[...l,s])},[Fe,Dt]=a.useState(()=>{if(typeof window<"u"){const s=sessionStorage.getItem("tiger_selectedStartPhase");return s?Number(s):0}return 0});a.useEffect(()=>{typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase",String(Fe))},[Fe]);const[dt,Kt]=a.useState(!0),[Ze,_t]=a.useState(!0),[R,Ie]=a.useState(null),[Ct,fn]=a.useState(null),[we,$t]=a.useState(null),[de,Ye]=a.useState(null),[It,ut]=a.useState(""),[ze,Mt]=a.useState([]);a.useEffect(()=>{I("http://localhost:5006/api/projects").then(s=>s?s.json():null).then(s=>{if(s&&s.success&&Array.isArray(s.projects))ie(s.projects.map(l=>({name:typeof l=="string"?l:l.project_id||l.title||l.name||"Projet",desc:"Projet local",bg:""})));else{const l=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:l?{Authorization:`Bearer ${l}`}:{}}).then(c=>c?c.json():null).then(c=>{c&&c.projects&&ie(c.projects.map(P=>({name:typeof P=="string"?P:P.project_id||P.title||P.name||"Projet",desc:"SaaS Cloud",bg:""})))}).catch(()=>{})}}).catch(()=>{})},[]);const[Ge,je]=a.useState(null),[wt,pt]=a.useState(""),[Ee,on]=a.useState(!1),[et,He]=a.useState(!1),i=a.useRef(null);a.useEffect(()=>{if(i.current=null,R==="v0-guest"||R==="V0-Guest")He(!0),je("http://localhost:3007"),pt("http://localhost:3007"),i.current="http://localhost:3007";else if(R){He(!0);const s=we&&JSON.stringify(we).includes("next.config"),l=typeof window<"u"&&window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1",c=s?"http://localhost:3000":i.current||(l?`http://${window.location.hostname}:5173`:"http://localhost:5173");je(c),pt(c),i.current=c}else He(!1),je(null)},[R,we]),a.useEffect(()=>{const s=l=>{var c,P,h,x;if(((c=l.data)==null?void 0:c.type)==="CHANGE_PREVIEW_URL"){const k=l.data.route,D=k==="/"?"":`#${k}`;je(ne=>{const Z=ne?ne.split("#")[0]:"http://127.0.0.1:5175/";return`${Z.endsWith("/")?Z:`${Z}/`}${D}`})}if(((P=l.data)==null?void 0:P.type)==="DESIGN_ELEMENT_CLICKED"||((h=l.data)==null?void 0:h.type)==="DESIGN_ELEMENT_DRAGGED"||((x=l.data)==null?void 0:x.type)==="DESIGN_ELEMENT_RESIZED"){const k=document.querySelector('iframe[title="Studio Admin Design"]');k&&k.contentWindow&&k.contentWindow.postMessage(l.data,"*")}};return window.addEventListener("message",s),()=>window.removeEventListener("message",s)},[]),a.useEffect(()=>{R&&I(`http://localhost:5006/api/fs/tree?project=${R}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&$t(s.tree)}).catch(()=>{})},[R]),a.useEffect(()=>{R&&de&&I(`http://localhost:5006/api/fs/read?project=${R}&file=${encodeURIComponent(de)}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&ut(s.content)}).catch(()=>{})},[R,de]);const g=async s=>{if(s===void 0||!R||!de)return;if(ut(s),Ft.isNativePlatform()||!!window.AndroidBridge)try{await Vn.writeFile({path:`v0-moteur-mobile/projetv0/${R}/${de}`,data:s,directory:pn.Documents,encoding:In.UTF8,recursive:!0})}catch(c){console.error("Erreur de sauvegarde mobile:",c)}else I("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:R,file:de,content:s})}).catch(()=>{})},b=s=>{R&&I(`http://localhost:5006/api/fs/read?project=${R}&file=${encodeURIComponent(s)}`).then(l=>l?l.json():null).then(l=>{l&&l.success&&Mt(c=>c.find(P=>P.path===s)?c:[...c,{path:s,content:l.content}])}).catch(()=>{})};a.useEffect(()=>{(async()=>{const l=["bg-gradient-to-br from-[#bf6969]/80 to-[#c27042]/90 backdrop-blur-md","bg-gradient-to-br from-[#a387b9]/80 to-[#aa6b73]/90 backdrop-blur-md","bg-gradient-to-br from-[#e4a37f]/80 to-[#bf6969]/90 backdrop-blur-md","bg-gradient-to-br from-[#aa6b73]/80 to-[#c27042]/90 backdrop-blur-md"];if(Ft.isNativePlatform()||!!window.AndroidBridge)try{const h=(await Vn.readdir({path:"v0-moteur-mobile/projetv0",directory:pn.Documents})).files.map(x=>x.name||x.toString());ie(h.map((x,k)=>({name:x,desc:"Mémoire Téléphone",bg:l[k%l.length]})))}catch{console.log("Aucun projet mobile trouvé ou dossier inexistant.")}else I("http://localhost:5006/api/projects-v2").then(P=>P?P.json():null).then(P=>{if(P&&P.success&&P.projects)ie(P.projects.map((h,x)=>({name:typeof h=="string"?h:h.name||h.project_id||h.title||`Projet_${x}`,desc:"Environnement Local",bg:l[x%l.length],installed:h.installed})));else{const h=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:h?{Authorization:`Bearer ${h}`}:{}}).then(x=>x?x.json():null).then(x=>{x&&x.projects&&ie(x.projects.map((k,D)=>({name:typeof k=="string"?k:k.project_id||k.title||k.name||`Projet_${D}`,desc:"SaaS Cloud (Neon DB)",bg:l[D%l.length]})))}).catch(()=>{})}}).catch(()=>{})})()},[]),a.useEffect(()=>{G(!0)},[]);const[N,Q]=a.useState(!0);a.useEffect(()=>{const s=()=>Q(!0);window.addEventListener("open-mouchard",s);const l=c=>{if(c.data&&c.data.type==="TIGER_CAPTURE"){const P=c.data.data;if(P){let h="index.html";(P.includes("import React")||P.includes("export default"))&&(h="App.tsx"),P.includes("tailwindcss")&&(h="index.css"),Ye(h),g(P),$t(x=>{var ne;const k={name:h,path:h,type:"file"};return x?((ne=x.children)==null?void 0:ne.find(Z=>Z.name===h))?x:{...x,children:[...x.children||[],k]}:{name:"Mobile_Storage",type:"directory",children:[k]}})}}};return window.addEventListener("message",l),()=>{window.removeEventListener("open-mouchard",s),window.removeEventListener("message",l)}},[R,de]),a.useEffect(()=>{if(!E)return;const s=()=>{I("http://localhost:5006/api/bridge/queue").then(c=>c?c.json():null).then(c=>{c&&c.success&&(setBridgeQueueData({current:c.current||{},queue:c.queue||[]}),(c.queue&&c.queue.length>0||c.current&&Object.keys(c.current).length>0)&&setIsPipelineRunning(!0))}).catch(()=>{}),I("http://localhost:5006/api/bridge/logs").then(c=>c?c.json():null).then(c=>{var P,h;if(c&&c.success&&c.logs){he(c.logs);const x=[...c.logs].reverse().find(D=>D.includes("URL_PREVIEW="));if(x){let D=(P=x.split("URL_PREVIEW=")[1])==null?void 0:P.trim();D&&typeof window<"u"&&window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1"&&(D=D.replace(/localhost|127\.0\.0\.1/,window.location.hostname)),D&&D!==i.current&&(i.current=D,je(D),pt(D),He(!0))}const k=[...c.logs].reverse().find(D=>D.includes("PIPELINE COMPLET TERMINÉ avec succès pour "));if(k){const D=(h=k.split("PIPELINE COMPLET TERMINÉ avec succès pour ")[1])==null?void 0:h.replace(/[.\s]+$/,"");D&&R!==D&&(Ie(D),He(!0))}}}).catch(()=>{}),I("http://localhost:5006/api/bridge/autonomous-status").then(c=>c?c.json():null).then(c=>{if(c&&c.success&&(c.data||c.status)){const h=(c.data||c.status).state;h==="diagnosing"||h==="repair_required"||h==="error"?S("error"):["repair_planned","patching_staging","validating","browser_verifying","repairing","working"].includes(h)||["starting_server","launch_requested","restarting"].includes(h)?S("working"):h==="ready"?S("clean"):R&&I(`http://localhost:5006/api/suture/status?project=${R}`).then(x=>x?x.json():null).then(x=>{x&&x.success&&x.status&&S(x.status)}).catch(()=>{})}}).catch(()=>{})};s();const l=setInterval(s,2e3);return()=>clearInterval(l)},[E,R]);const T=s=>{const l=typeof s=="string"?s:v;if(!l.trim())return;const c={id:Date.now().toString(),role:"user",content:l};_(h=>[...h,c]);const P=l.toLowerCase();setTimeout(()=>{let h={id:(Date.now()+1).toString(),role:"assistant",content:""};const x=P.normalize("NFD").replace(/[\u0300-\u036f]/g,"");if(x==="mes projets"||x.includes("liste des projet")||x==="projet"||x==="projets")h.content="Voici la liste de vos projets récents :",h.widget="projects";else if((x.includes("stitch")||x.includes("deepseek")||x.includes("design")||x.includes("logique"))&&(x.includes("modifi")||x.includes("ajoute")||x.includes("change")||x.includes("mise a jour")||x.includes("evolue")||x.includes("reprends")||x.includes("continue"))){const k=l.match(/\[(.*?)\]/),D=k?k[1]:null,ne=x.includes("deepseek")?"deepseek":"stitch";h.content=`🔄 MODE ÉVOLUTION ACTIVÉ (${ne.toUpperCase()}) 🔄

J'injecte vos nouvelles directives directement dans votre interface... ${D?`
🔍 Recherche et AutoSwitch vers le projet : "${D}"`:"Reprise du travail en cours."}`,h.widget=null,I("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:ne,target_project:D,prompt:"Mise à jour (Reprise de projet) : "+l.replace(/\[.*?\]/,"").trim(),auto_submit:!0})}).catch(Z=>console.log("Erreur de connexion au Bridge local pour l'injection",Z))}else if(x.includes("cree")||x.includes("lance")||x.includes("projet")||x.includes("generation")){h.content=`🚀 DÉMARRAGE PARALLÈLE KIROV5 🚀
...
Les intelligences artificielles sont informées et en attente. Une fois le design terminé, glissez l'HTML ici pour lancer le câblage final en phase 5.`,h.widget="phases",z(1);const k=R||(me.trim()?me.trim().replace(/[^a-zA-Z0-9_-]/g,"_"):"Projet_"+l.substring(0,15).replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4));if(Ie(k),typeof window<"u"&&localStorage.setItem("tiger_lastGeneratedProject",k),typeof window<"u"){const Z=localStorage.getItem("tiger_targetAi")||"deepseek",Y=localStorage.getItem("tiger_targetUiAi")||"stitch",Te=(O,le=!1)=>st(O,le),De=window.AndroidBridge;if(De&&De.openAIWithPrompt)De.openAIWithPrompt(Te(Y,!0),"Génère l'interface UI/UX complète et moderne pour ce projet : "+l),De.showToast&&De.showToast("Stitch s'ouvre. Générez le HTML, puis utilisez le Trombone.");else{const O=j&&j.length>0?`

[PACKS PRD ARCHITECTURE SELECTIONNES (${j.length})]
`+j.map(ve=>{const re=Lt.find(xe=>xe.id===ve);return`• ${re?re.name:ve} (#${ve})`}).join(`
`):"",le="Génère l'interface UI/UX complète et moderne pour ce projet : "+l+O;try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(le)}catch{}const $e=()=>I("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Y,prompt:le,auto_submit:!0,project_id:k,phase_num:1,packs:j})});Fe!==200&&(Ze?window.open(Te(Y,!0),"kirov5_ai_target"):window.open(Te(Y,!0),"_blank"),$e().then(()=>{console.log("Prompt UI envoyé avec succès au Bridge pour Stitch !")}).catch(ve=>console.error("Erreur sendUiPrompt",ve))),setTimeout(()=>{const ve=j&&j.length>0?l+O:"L'interface UI/UX est actuellement en cours de génération. Prépare la structure backend et les états React pour un projet complexe : "+l+". Reste en attente, je te fournirai le fichier HTML pour le câblage final.";try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(ve)}catch{}window.open(Te(Z),"_blank"),I("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:ve,auto_submit:!0,project_id:k,phase_num:1,packs:j})}).catch(re=>console.log("Erreur Bridge Logic Prompt:",re)),j&&j.length>0?I("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,user_prompt:ve,packs:j,target_project:k})}).then(()=>{console.log("Méga-Prompt Trombone envoyé !")}).catch(re=>{console.log("Erreur Trombone Bridge:",re)}):I("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:ve,auto_submit:!0,project_id:k,phase_num:1})}).then(()=>{console.log("Prompt Logique envoyé au Bridge")}).catch(()=>{})},800),console.log("Les IA ont été ouvertes. Les prompts ont été envoyés au Bridge local pour injection via l'extension.")}}z(1);let D=1;const ne=setInterval(()=>{D++,D>11?clearInterval(ne):z(D)},1500)}else if(x.includes("youtube")||x.includes("video"))h.content="Voici les résultats YouTube pour votre recherche :",h.widget="youtube";else if(x.includes("actualite")||x.includes("ia")||x.includes("news")||x.includes("recherche")){const k=localStorage.getItem("tiger_apiKey");h.content="Actualités & Dernières Innovations IA :",h.widget="news",zt(k||void 0)}else if(x.includes("parametre")||x.includes("reglage")||x.includes("configuration")||x.includes("setting"))h.content="Ouverture du panneau de configuration système :",h.widget="settings";else if(x.includes("[plugin:vite:")||x.includes("missing semicolon")||x.includes("syntaxerror")||x.includes("typescriptparsermixin")||x.includes("babel/parser")||x.includes("error")&&x.length>80){const k=R||"Projet_blog_8831";h.content=`🚨 Trace d'erreur détectée !

Lancement immédiat de l'Auto-Suture IA pour le projet "${k}" avec la stack trace fournie... 🩺`,Pe("suture",l)}else if(h.content="Traitement de votre demande via Tiger IA...",typeof window<"u"){const k=window.AndroidBridge;k&&k.openAIWithPrompt?k.openAIWithPrompt("https://chat.deepseek.com/",l):window.open("https://chat.deepseek.com/","_blank")}_(k=>[...k,h])},600),f("")},ue=async s=>{var De;let l=Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s],c=null;const P=l.filter(O=>O.name.endsWith(".zip"));let h=[];if(P.length>0){_(O=>[...O,{id:Date.now().toString()+"_zip",role:"user",content:"📦 Extraction de l'archive ZIP en cours..."}]);try{const O=(await Xt(async()=>{const{default:le}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:le}},[])).default;for(const le of P){c=le;const ve=await new O().loadAsync(le);for(const[re,xe]of Object.entries(ve.files))if(!xe.dir&&!re.includes("__MACOSX")&&!((De=re.split("/").pop())!=null&&De.startsWith("."))){const ke=await xe.async("blob"),nt=new File([ke],re.split("/").pop()||re,{type:ke.type});h.push(nt)}}}catch(O){console.error("Erreur décompression ZIP:",O),alert("Système Kirov5 : Impossible de lire l'archive ZIP.")}for(const O of P)try{const le=await O.arrayBuffer(),$e=arrayBufferToBase64(le);fetch("/api/fs/upload-zip",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:R||U||me||"AUDIO",fileName:O.name,fileBase64:$e})}).catch(re=>console.warn("[Upload ZIP Sync] Notice:",re))}catch{}}l=[...l.filter(O=>!O.name.endsWith(".zip")),...h];const x=l.filter(O=>O.name.endsWith(".html")),k=l.filter(O=>!O.name.endsWith(".html")&&(O.name.endsWith(".png")||O.name.endsWith(".jpg")||O.name.endsWith(".jpeg")||O.name.endsWith(".md")||O.name.endsWith(".json")||O.name.endsWith(".txt")||O.name.endsWith(".tsx")||O.name.endsWith(".ts")||O.name.endsWith(".jsx")||O.name.endsWith(".js")||O.name.endsWith(".css")||O.name.endsWith(".svg")));if(x.length===0&&k.length===0){alert("Système Kirov5 : Format non supporté. Veuillez déposer au moins un .html, .md, .tsx, .ts, .png, ou un .zip.");return}let D=[...ze];for(const O of k){const le=O.name.endsWith(".png")||O.name.endsWith(".jpg")||O.name.endsWith(".jpeg"),$e=await new Promise(re=>{const xe=new FileReader;xe.onload=ke=>{var nt;return re((nt=ke.target)==null?void 0:nt.result)},le?xe.readAsDataURL(O):xe.readAsText(O)}),ve={path:O.name,content:le?`[Image jointe visuellement : ${O.name} (Binaire ignoré pour économie de tokens)]`:$e};D.some(re=>re.path===O.name)||D.push(ve),_(re=>[...re,{id:Date.now().toString()+"_"+Math.random(),role:"user",content:`📎 Fichier de contexte ajouté au Trombone : ${O.name}`}])}if(x.length>0){const O=x[0],le=await new Promise(re=>{const xe=new FileReader;xe.onload=ke=>{var nt;return re((nt=ke.target)==null?void 0:nt.result)},xe.readAsText(O)}),$e={path:O.name,content:le};D.some(re=>re.path===O.name)||D.push($e);const ve={id:Date.now().toString()+"_main",role:"user",content:`📁 Fichier de design déposé : ${O.name}
Analyse de la structure UI et assemblage du contexte en cours...`};_(re=>[...re,ve])}const ne=(Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s]).filter(O=>O.name.endsWith(".zip"));for(const O of ne)D.some(le=>le.path===O.name)||D.push({path:O.name,content:"ZIP_ARCHIVE_DUMMY"});Mt(D);const Z=R||me||"Projet_Stitch",Y=l.map(O=>O.name).join(", ");console.log(`[TROMBONE] Fichiers déposés pour ${Z}: ${Y}`);const Te=x.length;if(Te>3){const O=localStorage.getItem("tiger_targetAi")||"deepseek",le=c?"Projet_ZIP_"+c.name.replace(/[^a-zA-Z0-9]/g,"_").replace(".zip","")+"_"+Date.now().toString().slice(-4):"Projet_BATCH_"+Date.now().toString().slice(-4),$e=R||le;R||Ie($e),_(re=>[...re,{id:Date.now().toString()+"_batch",role:"assistant",content:`🤖 KIROV5 MULTI-BATCH DÉTECTÉ !

Hermes a détecté ${Te} pages HTML dans votre ZIP.
Découpage automatique en lots de 3 fichiers en cours...
Démarrage de l'automatisation séquentielle via DeepSeek !`,widget:"phases"}]),z(1);let ve="";if(c)ve=await new Promise(re=>{const xe=new FileReader;xe.onload=ke=>{var nt;return re(((nt=ke.target)==null?void 0:nt.result).split(",")[1])},xe.readAsDataURL(c)});else{const re=(await Xt(async()=>{const{default:ke}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:ke}},[])).default,xe=new re;for(const ke of l)xe.file(ke.name,ke);ve=await xe.generateAsync({type:"base64"})}I("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({zip_base64:ve,zip_mode:!0,target_ai:O,target_project:$e,user_prompt:me.trim()?`${me} - ${lt}`:`Application multi-pages issue de ${(c==null?void 0:c.name)||"fichiers_locaux"}`})}).then(async re=>{const xe=await re.json();xe.success?(_(ke=>[...ke,{id:Date.now().toString()+"_hermes",role:"assistant",content:`✅ ${xe.message}

📡 Lot 1/${xe.total_batches} transmis à DeepSeek. Les lots suivants s'enchaîneront automatiquement !
Fichiers détectés : ${(xe.files_detected||[]).join(", ")}`}]),window.open(st(O),"_blank")):alert("Erreur Hermes : "+(xe.error||"Inconnue"))}).catch(re=>{alert(`Erreur de connexion au Moteur Local : ${re.message}`)});return}if(x.length>0){if(document.getElementById("creation-mode-container"))return;setTimeout(()=>{Ae(D)},1200)}},Ae=async(s=ze)=>{rt(!1);const l=s.find(Z=>Z.path.endsWith(".html"));if(!l){alert("Aucun fichier HTML trouvé dans le Trombone !");return}const c=me.trim()?"Projet_"+me.replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4):"Design_"+l.path.replace(".html","").replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4),P=R||c;if(!R){Ie(P);try{await I("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:P,file:"README.md",content:`# ${me||P}

Initialisé par Tiger IA V0 (Design-First).`})})}catch(Z){console.error("[IDE] Erreur création auto du dossier",Z)}}const h=s.filter(Z=>Z.path!==l.path),x=l.content,k={id:(Date.now()+1).toString(),role:"assistant",content:`Mode "Design-First" activé. 🎨

Transmission du design au LLM avec les ${h.length} fichiers du Trombone intégrés.
Câblage en cours...`,widget:"phases"};z(5),_(Z=>[...Z,k]);let D=5;const ne=setInterval(()=>{D++,D>11?clearInterval(ne):z(D)},3e3);if(typeof window<"u"&&window.AndroidBridge&&window.AndroidBridge.showToast&&window.AndroidBridge.showToast("Mode Design-First Activé !"),typeof window<"u"){const Z=localStorage.getItem("tiger_targetAi")||"deepseek";let Y="";h.length>0&&(Y=`
...
\`\`\`
`,O=window.AndroidBridge;if(O&&O.openAIWithPrompt){const le=st(Z);O.openAIWithPrompt(le,De),O.showToast&&O.showToast("HTML injecté. Câblage sur "+Z.toUpperCase()+" !")}else I("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:De,auto_submit:!0,project_id:P,phase_num:5})}).then(le=>console.log("[Bridge] Prompt HTML + Contexte envoyé avec succès, status:",le.status)).catch(le=>console.error("[Bridge] Erreur lors de l'envoi du prompt:",le))}},tt=(s,l=0)=>s?s.type==="directory"?e.jsxs("div",{className:"flex flex-col",children:[e.jsxs("div",{className:"flex items-center gap-2 px-2 py-1 hover:bg-white/5 cursor-pointer text-gray-300 text-sm",style:{paddingLeft:`${l*12+8}px`},children:[e.jsx("span",{className:"text-orange-400",children:"📁"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-dossier truncate font-bold",children:s.name})]}),s.children&&s.children.map(c=>tt(c,l+1))]},s.path):e.jsxs("div",{className:`flex items-center justify-between px-2 py-1 hover:bg-white/10 cursor-pointer text-sm group ${de===s.path?"bg-cyan/20 text-cyan border-l-2 border-cyan":"text-gray-400"}`,style:{paddingLeft:`${l*12+8}px`},onClick:()=>Ye(s.path),children:[e.jsxs("div",{className:"flex items-center gap-2 truncate",children:[e.jsx("span",{className:"text-blue-400",children:"📄"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-fichier truncate",children:s.name})]}),e.jsxs("div",{className:"hidden group-hover:flex items-center gap-1",children:[e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-red-500/50 transition-colors",title:"Suture : Réparer ou débugger ce fichier",onClick:c=>{c.stopPropagation(),Ye(s.path),setTimeout(()=>Pe("suture","Veuillez analyser ce fichier et corriger toutes les erreurs, fautes de frappe ou imports manquants."),100)},children:"🩺"}),e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-cyan/50 transition-colors",title:"Ajouter au Trombone",onClick:c=>{c.stopPropagation(),b(s.path)},children:"📎"})]})]},s.path):null,Pe=async(s,l)=>{if(!R){alert("⚠️ Veuillez sélectionner un projet actif dans l'explorateur.");return}const c=localStorage.getItem("tiger_targetAi")||"deepseek";let P="";s==="suture"?P=`🩺 SUTURE CHIRURGICALE — CORRECTION D'ERREUR DÉTECTÉE
...
`):s==="refactor"?P=`🔄 REFACTORING — Refactorise le code du projet [${R}] pour une qualité industrielle.

`:s==="improve"&&(P=`✨ AMÉLIORATION — Ajoute des animations et améliorations UI pour le projet [${R}].

`),de&&It&&(P+=`--- Fichier Actif en Édition : ${de} ---
\`\`\`
${It}
\`\`\`

`),ze.length>0&&(P+=`--- Fichiers de contexte du Trombone ---

`,ze.forEach(h=>{P+=`--- ${h.path} ---
\`\`\`
${h.content}
\`\`\`
...
`:Le.includes("Canvas 2D")?D+=`⚠️ CONSTRUCTEUR : Ce projet DOIT IMPÉRATIVEMENT être un jeu Web 2D généré en **React + Vite + TypeScript (TSX) + Canvas 2D**.
