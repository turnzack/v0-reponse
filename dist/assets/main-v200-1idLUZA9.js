const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-v200-BuXwLrKk.js","assets/design-v200-DcgYHDqz.js","assets/design-v200-B_-ohhET.css"])))=>i.map(i=>d[i]);
...
L'orchestrateur prend le relais pour générer le Backend et intégrer les composants.`:"Mode Cloud SaaS : Bridge non disponible.")}).catch(N=>{console.error(N),alert("Erreur: Le moteur n'a pas pu être contacté.")});return}if(typeof window<"u"&&(window.open("https://stitch.withgoogle.com/","_blank"),window.dispatchEvent(new CustomEvent("open-mouchard"))),Number(A)===0){w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_project:v,target_ai:"stitch",start_index:1,zip_mode:!0,start_phase:0,auto_pilot:ee,instructions:it||"Génère l'application avec ces packs et directives.",packs:te})}).then(N=>N?N.json():null).then(N=>{var pt;const me=(N==null?void 0:N.prompt)||((pt=N==null?void 0:N.data)==null?void 0:pt.prompt)||"Génération UI/UX Stitch complète.",_e=te.length>0?te.join(", "):"standard";if(T(2),typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase","2"),typeof document<"u"){const Le=document.getElementById("tiger-mega-prompt-overlay");Le&&Le.remove();const et=document.createElement("div");et.id="tiger-mega-prompt-overlay",et.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);z-index:999999;display:flex;align-items:center;justify-content:center;padding:16px;font-family:sans-serif;",et.innerHTML=`
                              ...
                              `,document.body.appendChild(et);const Ct=document.getElementById("tiger-prompt-area"),Fe=document.getElementById("btn-copy-prompt"),Ge=document.getElementById("btn-close-prompt-overlay"),tt=document.getElementById("btn-open-stitch"),Bt=()=>{Ct&&(Ct.select(),Ct.setSelectionRange(0,99999),document.execCommand("copy")),navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),T(2),Fe&&(Fe.innerHTML="✅ COPIÉ DANS LE PRESSE-PAPIER !",Fe.style.background="linear-gradient(135deg,#10b981,#059669)",setTimeout(()=>{Fe&&(Fe.innerHTML="📋 COPIER LE MÉGA-PROMPT",Fe.style.background="linear-gradient(135deg,#06b6d4,#2563eb)")},3e3))};Fe&&(Fe.onclick=()=>{Bt(),T(2)}),Ge&&(Ge.onclick=()=>{et.remove(),T(2)}),tt&&(tt.onclick=()=>{window.open("https://stitch.withgoogle.com/","_blank"),T(2)}),Bt(),T(2)}}).catch(N=>{console.error(N),alert("Erreur de communication avec le serveur Kirov5.")});return}w("/api/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:"stitch",user_prompt:it||"Génère l'application avec ces packs et directives.",packs:te,target_project:v,phase_num:1})}).then(N=>N?N.json():null).then(N=>{var _e;const me=(N==null?void 0:N.prompt)||((_e=N==null?void 0:N.data)==null?void 0:_e.prompt)||"Génération UI/UX Stitch.";typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),alert(`🚀 PHASE 1 (STITCH UI/UX) INITIALISÉE !
...
Câblage en cours...`,widget:"phases"};E(5),C(J=>[...J,j]);let M=5;const Z=setInterval(()=>{M++,M>11?clearInterval(Z):E(M)},3e3);if(typeof window<"u"&&window.AndroidBridge&&window.AndroidBridge.showToast&&window.AndroidBridge.showToast("Mode Design-First Activé !"),typeof window<"u"){const J=localStorage.getItem("tiger_targetAi")||"deepseek";let V="";u.length>0&&(V=`
