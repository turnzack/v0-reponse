const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-v200-Aie2WzOG.js","assets/design-v200-BU4g7UhC.js","assets/design-v200-CHRXnMOk.css"])))=>i.map(i=>d[i]);
...
L'orchestrateur prend le relais pour générer le Backend et intégrer les composants.`:"Mode Cloud SaaS : Bridge non disponible.")}).catch(k=>{console.error(k),alert("Erreur: Le moteur n'a pas pu être contacté.")});return}typeof window<"u"&&(window.open("https://stitch.withgoogle.com/","_blank"),window.dispatchEvent(new CustomEvent("open-mouchard")));const ne=window.KIROV_SELECTED_PACKS&&window.KIROV_SELECTED_PACKS.length>0?window.KIROV_SELECTED_PACKS:q&&q.length>0?q:[];if(Number(x)===0){I("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_project:v,target_ai:"stitch",start_index:1,zip_mode:!0,start_phase:0,auto_pilot:Q,instructions:st||"Génère l'application avec ces packs et directives.",packs:ne})}).then(k=>k?k.json():null).then(k=>{var Ve;const me=(k==null?void 0:k.prompt)||((Ve=k==null?void 0:k.data)==null?void 0:Ve.prompt)||"Génération UI/UX Stitch complète.",_e=ne.length>0?ne.join(", "):"standard";if(w(2),typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase","2"),typeof document<"u"){const Qe=document.getElementById("tiger-mega-prompt-overlay");Qe&&Qe.remove();const tt=document.createElement("div");tt.id="tiger-mega-prompt-overlay",tt.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);z-index:999999;display:flex;align-items:center;justify-content:center;padding:16px;font-family:sans-serif;",tt.innerHTML=`
                              ...
                              `,document.body.appendChild(tt);const gt=document.getElementById("tiger-prompt-area"),Pe=document.getElementById("btn-copy-prompt"),yt=document.getElementById("btn-close-prompt-overlay"),Ze=document.getElementById("btn-open-stitch"),Ut=()=>{gt&&(gt.select(),gt.setSelectionRange(0,99999),document.execCommand("copy")),navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),w(2),Pe&&(Pe.innerHTML="✅ COPIÉ DANS LE PRESSE-PAPIER !",Pe.style.background="linear-gradient(135deg,#10b981,#059669)",setTimeout(()=>{Pe&&(Pe.innerHTML="📋 COPIER LE MÉGA-PROMPT",Pe.style.background="linear-gradient(135deg,#06b6d4,#2563eb)")},3e3))};Pe&&(Pe.onclick=()=>{Ut(),w(2)}),yt&&(yt.onclick=()=>{tt.remove(),w(2)}),Ze&&(Ze.onclick=()=>{window.open("https://stitch.withgoogle.com/","_blank"),w(2)}),Ut(),w(2)}}).catch(k=>{console.error(k),alert("Erreur de communication avec le serveur Kirov5.")});return}I("/api/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:"stitch",user_prompt:st||"Génère l'application avec ces packs et directives.",packs:ne,target_project:v,phase_num:1})}).then(k=>k?k.json():null).then(k=>{var _e;const me=(k==null?void 0:k.prompt)||((_e=k==null?void 0:k.data)==null?void 0:_e.prompt)||"Génération UI/UX Stitch.";typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),alert(`🚀 PHASE 1 (STITCH UI/UX) INITIALISÉE !
...
`+j.map(Ce=>{const ie=Ft.find(Se=>Se.id===Ce);return`• ${ie?ie.name:Ce} (#${Ce})`}).join(`
...
`),pe&&Lt&&(C+=`--- Fichier Actif en Édition : ${pe} ---
\`\`\`
${Lt}
\`\`\`
...
[PACKS PRD ARCHITECTURE SELECTIONNES (${j.length})]
`+j.map(J=>{const qe=Ft.find(Be=>Be.id===J);return`• ${qe?qe.name:J} (#${J})`}).join(`
`):"";let R=`
