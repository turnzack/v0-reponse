const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-v200-CZ9DyCJy.js","assets/design-v200-DcgYHDqz.js","assets/design-v200-B_-ohhET.css"])))=>i.map(i=>d[i]);
...
L'orchestrateur prend le relais pour générer le Backend et intégrer les composants.`:"Mode Cloud SaaS : Bridge non disponible.")}).catch(N=>{console.error(N),alert("Erreur: Le moteur n'a pas pu être contacté.")});return}typeof window<"u"&&(window.open("https://stitch.withgoogle.com/","_blank"),window.dispatchEvent(new CustomEvent("open-mouchard")));const re=window.KIROV_SELECTED_PACKS&&window.KIROV_SELECTED_PACKS.length>0?window.KIROV_SELECTED_PACKS:k&&k.length>0?k:[];if(Number(A)===0){w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_project:v,target_ai:"stitch",start_index:1,zip_mode:!0,start_phase:0,auto_pilot:ee,instructions:it||"Génère l'application avec ces packs et directives.",packs:re})}).then(N=>N?N.json():null).then(N=>{var pt;const me=(N==null?void 0:N.prompt)||((pt=N==null?void 0:N.data)==null?void 0:pt.prompt)||"Génération UI/UX Stitch complète.",_e=re.length>0?re.join(", "):"standard";if(E(2),typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase","2"),typeof document<"u"){const Le=document.getElementById("tiger-mega-prompt-overlay");Le&&Le.remove();const et=document.createElement("div");et.id="tiger-mega-prompt-overlay",et.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);z-index:999999;display:flex;align-items:center;justify-content:center;padding:16px;font-family:sans-serif;",et.innerHTML=`
                              ...
                              `,document.body.appendChild(et);const Ct=document.getElementById("tiger-prompt-area"),Fe=document.getElementById("btn-copy-prompt"),Ge=document.getElementById("btn-close-prompt-overlay"),tt=document.getElementById("btn-open-stitch"),Bt=()=>{Ct&&(Ct.select(),Ct.setSelectionRange(0,99999),document.execCommand("copy")),navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),E(2),Fe&&(Fe.innerHTML="✅ COPIÉ DANS LE PRESSE-PAPIER !",Fe.style.background="linear-gradient(135deg,#10b981,#059669)",setTimeout(()=>{Fe&&(Fe.innerHTML="📋 COPIER LE MÉGA-PROMPT",Fe.style.background="linear-gradient(135deg,#06b6d4,#2563eb)")},3e3))};Fe&&(Fe.onclick=()=>{Bt(),E(2)}),Ge&&(Ge.onclick=()=>{et.remove(),E(2)}),tt&&(tt.onclick=()=>{window.open("https://stitch.withgoogle.com/","_blank"),E(2)}),Bt(),E(2)}}).catch(N=>{console.error(N),alert("Erreur de communication avec le serveur Kirov5.")});return}w("/api/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:"stitch",user_prompt:it||"Génère l'application avec ces packs et directives.",packs:re,target_project:v,phase_num:1})}).then(N=>N?N.json():null).then(N=>{var _e;const me=(N==null?void 0:N.prompt)||((_e=N==null?void 0:N.data)==null?void 0:_e.prompt)||"Génération UI/UX Stitch.";typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),alert(`🚀 PHASE 1 (STITCH UI/UX) INITIALISÉE !
...
Spécifications techniques du pack Guest ${A.name}.`})}},className:"design-prd-btn text-purple-400 text-xs font-bold hover:underline cursor-pointer flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-purple-500/30 transition-all",children:[e.jsx("span",{children:"📖"})," README →"]})]})]},E)})}):e.jsx(Kt,{items:$t.map(A=>{const E=A.id,k=A.name,f=A.icon||ps,b=t?t.includes(E):!1,P=(y[E]||fn(E)||"Spécifications techniques du contrat PRD.").replace(/^#+.*$/gm,"").replace(/^>.*$/gm,"").replace(/\[([^\]]+)\]\([^\)]+\)/g,"$1").replace(/[*_~`]/g,"").trim().split(`
...
`+R.map(Ae=>{const oe=$t.find(Ce=>Ce.id===Ae);return`• ${oe?oe.name:Ae} (#${Ae})`}).join(`
...
\`\`\`
`,$=window.AndroidBridge;if($&&$.openAIWithPrompt){const ue=gt(J);$.openAIWithPrompt(ue,Ie),$.showToast&&$.showToast("HTML injecté. Câblage sur "+J.toUpperCase()+" !")}else w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:J,prompt:Ie,auto_submit:!0,project_id:_,phase_num:5})}).then(ue=>console.log("[Bridge] Prompt HTML + Contexte envoyé avec succès, status:",ue.status)).catch(ue=>console.error("[Bridge] Erreur lors de l'envoi du prompt:",ue))}},Fe=(s,a=0)=>s?s.type==="directory"?e.jsxs("div",{className:"flex flex-col",children:[e.jsxs("div",{className:"flex items-center gap-2 px-2 py-1 hover:bg-white/5 cursor-pointer text-gray-300 text-sm",style:{paddingLeft:`${a*12+8}px`},children:[e.jsx("span",{className:"text-orange-400",children:"📁"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-dossier truncate font-bold",children:s.name})]}),s.children&&s.children.map(l=>Fe(l,a+1))]},s.path):e.jsxs("div",{className:`flex items-center justify-between px-2 py-1 hover:bg-white/10 cursor-pointer text-sm group ${fe===s.path?"bg-cyan/20 text-cyan border-l-2 border-cyan":"text-gray-400"}`,style:{paddingLeft:`${a*12+8}px`},onClick:()=>nt(s.path),children:[e.jsxs("div",{className:"flex items-center gap-2 truncate",children:[e.jsx("span",{className:"text-blue-400",children:"📄"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-fichier truncate",children:s.name})]}),e.jsxs("div",{className:"hidden group-hover:flex items-center gap-1",children:[e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-red-500/50 transition-colors",title:"Suture : Réparer ou débugger ce fichier",onClick:l=>{l.stopPropagation(),nt(s.path),setTimeout(()=>Ge("suture","Veuillez analyser ce fichier et corriger toutes les erreurs, fautes de frappe ou imports manquants."),100)},children:"🩺"}),e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-cyan/50 transition-colors",title:"Ajouter au Trombone",onClick:l=>{l.stopPropagation(),N(s.path)},children:"📎"})]})]},s.path):null,Ge=async(s,a)=>{if(!q){alert("⚠️ Veuillez sélectionner un projet actif dans l'explorateur.");return}const l=localStorage.getItem("tiger_targetAi")||"deepseek";let _="";s==="suture"?_=`🩺 SUTURE CHIRURGICALE — CORRECTION D'ERREUR DÉTECTÉE
...
[PACKS PRD ARCHITECTURE SELECTIONNES (${R.length})]
`+R.map(V=>{const xe=$t.find(Ie=>Ie.id===V);return`• ${xe?xe.name:V} (#${V})`}).join(`
`):"";let M=`
