const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-v200-C2ryf36F.js","assets/design-v200-DcgYHDqz.js","assets/design-v200-B_-ohhET.css"])))=>i.map(i=>d[i]);
...
L'orchestrateur prend le relais pour générer le Backend et intégrer les composants.`:"Mode Cloud SaaS : Bridge non disponible.")}).catch(N=>{console.error(N),alert("Erreur: Le moteur n'a pas pu être contacté.")});return}typeof window<"u"&&(window.open("https://stitch.withgoogle.com/","_blank"),window.dispatchEvent(new CustomEvent("open-mouchard")));const re=window.KIROV_SELECTED_PACKS&&window.KIROV_SELECTED_PACKS.length>0?window.KIROV_SELECTED_PACKS:k&&k.length>0?k:[];if(Number(A)===0){w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_project:v,target_ai:"stitch",start_index:1,zip_mode:!0,start_phase:0,auto_pilot:ee,instructions:it||"Génère l'application avec ces packs et directives.",packs:re})}).then(N=>N?N.json():null).then(N=>{var pt;const me=(N==null?void 0:N.prompt)||((pt=N==null?void 0:N.data)==null?void 0:pt.prompt)||"Génération UI/UX Stitch complète.",_e=re.length>0?re.join(", "):"standard";if(E(2),typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase","2"),typeof document<"u"){const Le=document.getElementById("tiger-mega-prompt-overlay");Le&&Le.remove();const et=document.createElement("div");et.id="tiger-mega-prompt-overlay",et.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);z-index:999999;display:flex;align-items:center;justify-content:center;padding:16px;font-family:sans-serif;",et.innerHTML=`
                              ...
                              `,document.body.appendChild(et);const Ct=document.getElementById("tiger-prompt-area"),Oe=document.getElementById("btn-copy-prompt"),Fe=document.getElementById("btn-close-prompt-overlay"),tt=document.getElementById("btn-open-stitch"),Bt=()=>{Ct&&(Ct.select(),Ct.setSelectionRange(0,99999),document.execCommand("copy")),navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),E(2),Oe&&(Oe.innerHTML="✅ COPIÉ DANS LE PRESSE-PAPIER !",Oe.style.background="linear-gradient(135deg,#10b981,#059669)",setTimeout(()=>{Oe&&(Oe.innerHTML="📋 COPIER LE MÉGA-PROMPT",Oe.style.background="linear-gradient(135deg,#06b6d4,#2563eb)")},3e3))};Oe&&(Oe.onclick=()=>{Bt(),E(2)}),Fe&&(Fe.onclick=()=>{et.remove(),E(2)}),tt&&(tt.onclick=()=>{window.open("https://stitch.withgoogle.com/","_blank"),E(2)}),Bt(),E(2)}}).catch(N=>{console.error(N),alert("Erreur de communication avec le serveur Kirov5.")});return}w("/api/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:"stitch",user_prompt:it||"Génère l'application avec ces packs et directives.",packs:re,target_project:v,phase_num:1})}).then(N=>N?N.json():null).then(N=>{var _e;const me=(N==null?void 0:N.prompt)||((_e=N==null?void 0:N.data)==null?void 0:_e.prompt)||"Génération UI/UX Stitch.";typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText(me).catch(()=>{}),alert(`🚀 PHASE 1 (STITCH UI/UX) INITIALISÉE !
...
3. Création du projet et câblage avec Stitch.`}];try{if(typeof window<"u"&&sessionStorage.getItem("deepseek_402")){at(a),ot(!1);return}const l=s||localStorage.getItem("tiger_apiKey");if(!l||!l.startsWith("sk-")||l.length<20||typeof window<"u"&&localStorage.getItem("deepseek_solde_epuise")==="true"){at(a),ot(!1);return}const _=new AbortController,u=setTimeout(()=>_.abort(),4e3),x=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${l}`},signal:_.signal,body:JSON.stringify({model:"deepseek-chat",messages:[{role:"system",content:"Tu es le fil d'actualités technologiques de l'OS Souverain. Réponds UNIQUEMENT en JSON avec une clé 'news' contenant 3 articles récents au format [{id, tag, title, desc, content}]."},{role:"user",content:"Donne 3 actualités récentes sur l'IA et le développement web."}],response_format:{type:"json_object"}})});if(clearTimeout(u),x.ok){const j=await x.json(),M=JSON.parse(j.choices[0].message.content);M.news&&Array.isArray(M.news)&&M.news.length>0?at(M.news):at(a)}else x.status===402&&typeof window<"u"&&(sessionStorage.setItem("deepseek_402","true"),localStorage.setItem("deepseek_solde_epuise","true")),at(a)}catch(l){console.warn("[Actualités] Erreur ou Timeout API DeepSeek, chargement fallback:",l),at(a)}finally{ot(!1)}},[]);o.useEffect(()=>{Mt()},[Mt]);const[Xe,lt]=o.useState(!1),[Me,qt]=o.useState(()=>localStorage.getItem("tiger_active_theme")||"fold"),[Ve,St]=o.useState([]),[Lt,un]=o.useState("");o.useEffect(()=>{const s=localStorage.getItem("tiger_saved_themes");if(s)try{let a=JSON.parse(s);if(!Array.isArray(a))throw new Error("Not an array");const l="linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)";a=a.map(_=>_.name==="fold"?{..._,colors:{..._.colors,"bg-app":l}}:_),St(a)}catch{St([])}else{const a={name:"fold",colors:{"icon-settings":"linear-gradient(135deg, #c87058, #934a36)","icon-ai":"linear-gradient(135deg, #9d508e, #622e5a)","icon-projects":"linear-gradient(135deg, #d38b5d, #a26038)","icon-packs":"linear-gradient(135deg, #445499, #252e66)","icon-news":"linear-gradient(135deg, #389eb2, #1f6475)","bg-app":"linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)"}};St([a]),localStorage.setItem("tiger_saved_themes",JSON.stringify([a]))}},[]),o.useEffect(()=>{var s;if(localStorage.setItem("tiger_active_theme",Me),Me!=="random"){const a=((s=Ve.find(l=>l.name===Me))==null?void 0:s.colors["bg-app"])||"linear-gradient(135deg, #1e1b4b 0%, #0f172a 50%, #000000 100%)";document.body.style.background=a,document.body.style.backgroundAttachment="fixed"}else document.body.style.background=""},[Me,Ve]);const we=()=>{if(!Lt.trim())return alert("Nom invalide");if(Ve.length>=10)return alert("Limite atteinte ! Vous ne pouvez sauvegarder que 10 thèmes maximum. Veuillez en supprimer un pour continuer.");const s={"icon-settings":c("icon-settings",.8),"icon-ai":c("icon-ai",.8),"icon-projects":c("icon-projects",.8),"icon-packs":c("icon-packs",.8),"icon-news":c("icon-news",.8),"bg-app":c("bg-app",.9)},a={name:Lt.trim(),colors:s},l=[...Ve,a];St(l),localStorage.setItem("tiger_saved_themes",JSON.stringify(l)),qt(a.name),un("")},_n=(s,a)=>{a.stopPropagation();const l=Ve.filter(_=>_.name!==s);St(l),localStorage.setItem("tiger_saved_themes",JSON.stringify(l)),Me===s&&qt("random")},Ut=s=>{Q(a=>a.includes(s)?a.filter(l=>l!==s):[...a,s])},[Qe,pn]=o.useState(()=>{if(typeof window<"u"){const s=sessionStorage.getItem("tiger_selectedStartPhase");return s?Number(s):0}return 0});o.useEffect(()=>{typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase",String(Qe))},[Qe]);const[Ot,Be]=o.useState(!0),[ze,ke]=o.useState(!0),[q,je]=o.useState(null),[He,mn]=o.useState(null),[qe,Ft]=o.useState(null),[fe,nt]=o.useState(null),[Gt,ct]=o.useState(""),[st,kt]=o.useState([]),[Ze,Xt]=o.useState(!1),[Ye,dt]=o.useState("idle"),[jt,$e]=o.useState([]),[yt,gn]=o.useState(""),[Tt,Vt]=o.useState(null),[ht,Cn]=o.useState([]),xt=o.useCallback(async()=>{try{let s=[];const a=Date.now();if(a-tn<3e4&&Kt.length>0)s=[...Kt];else try{const l=Ln(),_=await fetch("https://api.github.com/repos/turnzack/v0-reponse/releases",{headers:l?{Authorization:`Bearer ${l}`,Accept:"application/vnd.github.v3+json"}:{Accept:"application/vnd.github.v3+json"}});if(_.ok){const u=await _.json();if(Array.isArray(u)){const x=[];u.forEach(j=>{Array.isArray(j.assets)&&j.assets.forEach(M=>{var Z;(Z=M.name)!=null&&Z.endsWith(".apk")&&x.push({file:M.name,name:M.name.replace(".apk",""),sizeMb:(M.size/(1024*1024)).toFixed(1),url:M.browser_download_url})})}),Kt=x,tn=a,s=[...x]}}}catch{}try{const l=await w("http://localhost:5006/api/mobile/list-apks");if(l&&l.ok){const _=await l.json();_&&_.success&&Array.isArray(_.apks)&&(s=[...s,..._.apks])}else{const _=await fetch("/api/mobile/list-apks");if(_&&_.ok){const u=await _.json();u&&u.success&&Array.isArray(u.apks)&&(s=[...s,...u.apks])}}}catch{}if(s.length>0){const l=new Set,_=s.filter(u=>l.has(u.file.toLowerCase())?!1:(l.add(u.file.toLowerCase()),!0));Cn(_)}}catch(s){console.warn("[v0-apk] loadAvailableApks error",s)}},[]);o.useEffect(()=>{Ze&&xt()},[Ze,xt]),o.useEffect(()=>{let s,a=0;return Ye==="building"&&(s=setInterval(async()=>{try{const l=await w("http://localhost:5006/api/mobile/build-logs");if(l&&l.ok){a=0;const _=await l.json();_.logs&&_.logs.length>0&&$e(_.logs),_.isBuilding===!1&&_.result&&(_.result.success?(dt("success"),Vt(_.result.apkUrl)):dt("error"),clearInterval(s))}else a++,a>=3&&(clearInterval(s),dt("error"),$e(_=>[..._,`❌ Service de compilation mobile absent sur ce serveur (${l?l.status:"erreur réseau"}).`,"📱 Option 1 (Cloud) : Déclenchez le compilateur automatique Cloud Souverain.",`💻 Option 2 (Local) : Lancez 'python apk_builder.py --src ... --name ${yt||q||"app"} --build' dans E:\\v0reponses\\v0-apk.`]))}catch(l){console.warn("Polling APK error",l)}},1500)),()=>{s&&clearInterval(s)}},[Ye,yt,q]),o.useEffect(()=>{w("http://localhost:5006/api/projects").then(s=>s?s.json():null).then(s=>{if(s&&s.success&&Array.isArray(s.projects))te(s.projects.map(a=>({name:typeof a=="string"?a:a.project_id||a.title||a.name||"Projet",desc:"Projet local",bg:""})));else{const a=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:a?{Authorization:`Bearer ${a}`}:{}}).then(l=>l?l.json():null).then(l=>{l&&l.projects&&te(l.projects.map(_=>({name:typeof _=="string"?_:_.project_id||_.title||_.name||"Projet",desc:"SaaS Cloud",bg:""})))}).catch(()=>{})}}).catch(()=>{})},[]);const[ut,ye]=o.useState(null),[Qt,_t]=o.useState(""),[Et,i]=o.useState(!1),[g,v]=o.useState(!1),D=o.useRef(null);o.useEffect(()=>{if(D.current=null,q==="v0-guest"||q==="V0-Guest")v(!0),ye("http://localhost:3007"),_t("http://localhost:3007"),D.current="http://localhost:3007";else if(q){v(!0);const s=qe&&JSON.stringify(qe).includes("next.config"),a=typeof window<"u"&&window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1",l=s?"http://localhost:3000":D.current||(a?`http://${window.location.hostname}:5173`:"http://localhost:5173");ye(l),_t(l),D.current=l}else v(!1),ye(null)},[q,qe]),o.useEffect(()=>{const s=a=>{var l,_,u,x;if(((l=a.data)==null?void 0:l.type)==="CHANGE_PREVIEW_URL"){const j=a.data.route,M=j==="/"?"":`#${j}`;ye(Z=>{const J=Z?Z.split("#")[0]:"http://127.0.0.1:5175/";return`${J.endsWith("/")?J:`${J}/`}${M}`})}if(((_=a.data)==null?void 0:_.type)==="DESIGN_ELEMENT_CLICKED"||((u=a.data)==null?void 0:u.type)==="DESIGN_ELEMENT_DRAGGED"||((x=a.data)==null?void 0:x.type)==="DESIGN_ELEMENT_RESIZED"){const j=document.querySelector('iframe[title="Studio Admin Design"]');j&&j.contentWindow&&j.contentWindow.postMessage(a.data,"*")}};return window.addEventListener("message",s),()=>window.removeEventListener("message",s)},[]),o.useEffect(()=>{q&&w(`http://localhost:5006/api/fs/tree?project=${q}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&Ft(s.tree)}).catch(()=>{})},[q]),o.useEffect(()=>{q&&fe&&w(`http://localhost:5006/api/fs/read?project=${q}&file=${encodeURIComponent(fe)}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&ct(s.content)}).catch(()=>{})},[q,fe]);const re=async s=>{if(s===void 0||!q||!fe)return;if(ct(s),Jt.isNativePlatform()||!!window.AndroidBridge)try{await Xn.writeFile({path:`v0-moteur-mobile/projetv0/${q}/${fe}`,data:s,directory:vn.Documents,encoding:Tn.UTF8,recursive:!0})}catch(l){console.error("Erreur de sauvegarde mobile:",l)}else w("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:q,file:fe,content:s})}).catch(()=>{})},N=s=>{q&&w(`http://localhost:5006/api/fs/read?project=${q}&file=${encodeURIComponent(s)}`).then(a=>a?a.json():null).then(a=>{a&&a.success&&kt(l=>l.find(_=>_.path===s)?l:[...l,{path:s,content:a.content}])}).catch(()=>{})};o.useEffect(()=>{(async()=>{const a=["bg-gradient-to-br from-[#bf6969]/80 to-[#c27042]/90 backdrop-blur-md","bg-gradient-to-br from-[#a387b9]/80 to-[#aa6b73]/90 backdrop-blur-md","bg-gradient-to-br from-[#e4a37f]/80 to-[#bf6969]/90 backdrop-blur-md","bg-gradient-to-br from-[#aa6b73]/80 to-[#c27042]/90 backdrop-blur-md"];if(Jt.isNativePlatform()||!!window.AndroidBridge)try{const u=(await Xn.readdir({path:"v0-moteur-mobile/projetv0",directory:vn.Documents})).files.map(x=>x.name||x.toString());te(u.map((x,j)=>({name:x,desc:"Mémoire Téléphone",bg:a[j%a.length]})))}catch{console.log("Aucun projet mobile trouvé ou dossier inexistant.")}else w("http://localhost:5006/api/projects-v2").then(_=>_?_.json():null).then(_=>{if(_&&_.success&&_.projects)te(_.projects.map((u,x)=>({name:typeof u=="string"?u:u.name||u.project_id||u.title||`Projet_${x}`,desc:"Environnement Local",bg:a[x%a.length],installed:u.installed})));else{const u=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:u?{Authorization:`Bearer ${u}`}:{}}).then(x=>x?x.json():null).then(x=>{x&&x.projects&&te(x.projects.map((j,M)=>({name:typeof j=="string"?j:j.project_id||j.title||j.name||`Projet_${M}`,desc:"SaaS Cloud (Neon DB)",bg:a[M%a.length]})))}).catch(()=>{})}}).catch(()=>{})})()},[]),o.useEffect(()=>{P(!0)},[]);const[me,_e]=o.useState(!0);o.useEffect(()=>{const s=()=>_e(!0),a=_=>{_&&_.detail&&le(u=>[_.detail,...u.slice(0,49)])};window.addEventListener("open-mouchard",s),window.addEventListener("kirov-mouchard-log",a);const l=_=>{if(_.data&&_.data.type==="TIGER_CAPTURE"){const u=_.data.data;if(u){let x="index.html";(u.includes("import React")||u.includes("export default"))&&(x="App.tsx"),u.includes("tailwindcss")&&(x="index.css"),nt(x),re(u),Ft(j=>{var J;const M={name:x,path:x,type:"file"};return j?((J=j.children)==null?void 0:J.find(V=>V.name===x))?j:{...j,children:[...j.children||[],M]}:{name:"Mobile_Storage",type:"directory",children:[M]}})}}};return window.addEventListener("message",l),()=>{window.removeEventListener("open-mouchard",s),window.removeEventListener("kirov-mouchard-log",a),window.removeEventListener("message",l)}},[q,fe]),o.useEffect(()=>{if(!I)return;const s=()=>{w("http://localhost:5006/api/bridge/queue").then(l=>l?l.json():null).then(l=>{l&&l.success&&(setBridgeQueueData({current:l.current||{},queue:l.queue||[]}),(l.queue&&l.queue.length>0||l.current&&Object.keys(l.current).length>0)&&setIsPipelineRunning(!0))}).catch(()=>{}),w("http://localhost:5006/api/bridge/logs").then(l=>l?l.json():null).then(l=>{var _,u;if(l&&l.success&&l.logs){le(M=>{const J=[...M.filter(Ie=>(Ie.includes("[📱")||Ie.includes("v0-apk")||Ie.includes("[APK]"))&&!l.logs.includes(Ie)),...l.logs],V=new Set,xe=[];for(const Ie of J)V.has(Ie)||(V.add(Ie),xe.push(Ie));return xe.slice(0,50)});const x=[...l.logs].reverse().find(M=>M.includes("URL_PREVIEW="));if(x){let M=(_=x.split("URL_PREVIEW=")[1])==null?void 0:_.trim();M&&typeof window<"u"&&window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1"&&(M=M.replace(/localhost|127\.0\.0\.1/,window.location.hostname)),M&&M!==D.current&&(D.current=M,ye(M),_t(M),v(!0))}const j=[...l.logs].reverse().find(M=>M.includes("PIPELINE COMPLET TERMINÉ avec succès pour "));if(j){const M=(u=j.split("PIPELINE COMPLET TERMINÉ avec succès pour ")[1])==null?void 0:u.replace(/[.\s]+$/,"");M&&q!==M&&(je(M),v(!0))}}}).catch(()=>{}),w("http://localhost:5006/api/bridge/autonomous-status").then(l=>l?l.json():null).then(l=>{if(l&&l.success&&(l.data||l.status)){const u=(l.data||l.status).state;u==="diagnosing"||u==="repair_required"||u==="error"?f("error"):["repair_planned","patching_staging","validating","browser_verifying","repairing","working"].includes(u)||["starting_server","launch_requested","restarting"].includes(u)?f("working"):u==="ready"?f("clean"):q&&w(`http://localhost:5006/api/suture/status?project=${q}`).then(x=>x?x.json():null).then(x=>{x&&x.success&&x.status&&f(x.status)}).catch(()=>{})}}).catch(()=>{})};s();const a=setInterval(s,2e3);return()=>clearInterval(a)},[I,q]);const pt=s=>{je(s),nt(null),ct(""),i(!1),_e(!1),v(!1),w("http://localhost:5006/api/bridge/launch-project",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:s,open_explorer:!1})}).then(async a=>{if(a&&a.ok){const l=await a.json().catch(()=>null);l!=null&&l.previewUrl&&(ye(l.previewUrl),_t(l.previewUrl))}}).catch(a=>console.error("Erreur de lancement preview:",a)),w(`http://localhost:5006/api/fs/tree?project=${encodeURIComponent(s)}`).then(a=>a?a.json():null).then(a=>{if(a&&a.success&&a.tree){Ft(a.tree);const l=u=>{if(!u)return null;if(u.type==="file"&&(u.path==="src/App.tsx"||u.path==="src/App.jsx"||u.name==="App.tsx"||u.name==="index.html"))return u.path;if(u.children&&Array.isArray(u.children))for(const x of u.children){const j=l(x);if(j)return j}return null},_=l(a.tree);_&&nt(_)}}).catch(a=>console.error("Erreur chargement arborescence:",a))},Le=s=>{const a=typeof s=="string"?s:y;if(!a.trim())return;const l={id:Date.now().toString(),role:"user",content:a};C(u=>[...u,l]);const _=a.toLowerCase();setTimeout(()=>{let u={id:(Date.now()+1).toString(),role:"assistant",content:""};const x=_.normalize("NFD").replace(/[\u0300-\u036f]/g,"");if(x==="mes projets"||x.includes("liste des projet")||x==="projet"||x==="projets")u.content="Voici la liste de vos projets récents :",u.widget="projects";else if((x.includes("stitch")||x.includes("deepseek")||x.includes("design")||x.includes("logique"))&&(x.includes("modifi")||x.includes("ajoute")||x.includes("change")||x.includes("mise a jour")||x.includes("evolue")||x.includes("reprends")||x.includes("continue"))){const j=a.match(/\[(.*?)\]/),M=j?j[1]:null,Z=x.includes("deepseek")?"deepseek":"stitch";u.content=`🔄 MODE ÉVOLUTION ACTIVÉ (${Z.toUpperCase()}) 🔄

J'injecte vos nouvelles directives directement dans votre interface... ${M?`
🔍 Recherche et AutoSwitch vers le projet : "${M}"`:"Reprise du travail en cours."}`,u.widget=null,w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,target_project:M,prompt:"Mise à jour (Reprise de projet) : "+a.replace(/\[.*?\]/,"").trim(),auto_submit:!0})}).catch(J=>console.log("Erreur de connexion au Bridge local pour l'injection",J))}else if(x.includes("cree")||x.includes("lance")||x.includes("projet")||x.includes("generation")){u.content=`🚀 DÉMARRAGE PARALLÈLE KIROV5 🚀
...
Les intelligences artificielles sont informées et en attente. Une fois le design terminé, glissez l'HTML ici pour lancer le câblage final en phase 5.`,u.widget="phases",T(1);const j=q||(ce.trim()?ce.trim().replace(/[^a-zA-Z0-9_-]/g,"_"):"Projet_"+a.substring(0,15).replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4));if(je(j),typeof window<"u"&&localStorage.setItem("tiger_lastGeneratedProject",j),typeof window<"u"){const J=localStorage.getItem("tiger_targetAi")||"deepseek",V=localStorage.getItem("tiger_targetUiAi")||"stitch",xe=($,ue=!1)=>gt($,ue),Ie=window.AndroidBridge;if(Ie&&Ie.openAIWithPrompt)Ie.openAIWithPrompt(xe(V,!0),"Génère l'interface UI/UX complète et moderne pour ce projet : "+a),Ie.showToast&&Ie.showToast("Stitch s'ouvre. Générez le HTML, puis utilisez le Trombone.");else{const $=R&&R.length>0?`

[PACKS PRD ARCHITECTURE SELECTIONNES (${R.length})]
`+R.map(Ae=>{const oe=Ht.find(Ce=>Ce.id===Ae);return`• ${oe?oe.name:Ae} (#${Ae})`}).join(`
`):"",ue="Génère l'interface UI/UX complète et moderne pour ce projet : "+a+$;try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(ue)}catch{}const rt=()=>w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:V,prompt:ue,auto_submit:!0,project_id:j,phase_num:1,packs:R})});Qe!==200&&(ze?window.open(xe(V,!0),"kirov5_ai_target"):window.open(xe(V,!0),"_blank"),rt().then(()=>{console.log("Prompt UI envoyé avec succès au Bridge pour Stitch !")}).catch(Ae=>console.error("Erreur sendUiPrompt",Ae))),setTimeout(()=>{const Ae=R&&R.length>0?a+$:"L'interface UI/UX est actuellement en cours de génération. Prépare la structure backend et les états React pour un projet complexe : "+a+". Reste en attente, je te fournirai le fichier HTML pour le câblage final.";try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(Ae)}catch{}window.open(xe(J),"_blank"),w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:J,prompt:Ae,auto_submit:!0,project_id:j,phase_num:1,packs:R})}).catch(oe=>console.log("Erreur Bridge Logic Prompt:",oe)),R&&R.length>0?w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:J,user_prompt:Ae,packs:R,target_project:j})}).then(()=>{console.log("Méga-Prompt Trombone envoyé !")}).catch(oe=>{console.log("Erreur Trombone Bridge:",oe)}):w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:J,prompt:Ae,auto_submit:!0,project_id:j,phase_num:1})}).then(()=>{console.log("Prompt Logique envoyé au Bridge")}).catch(()=>{})},800),console.log("Les IA ont été ouvertes. Les prompts ont été envoyés au Bridge local pour injection via l'extension.")}}T(1);let M=1;const Z=setInterval(()=>{M++,M>11?clearInterval(Z):T(M)},1500)}else if(x.includes("youtube")||x.includes("video"))u.content="Voici les résultats YouTube pour votre recherche :",u.widget="youtube";else if(x.includes("actualite")||x.includes("ia")||x.includes("news")||x.includes("recherche")){const j=localStorage.getItem("tiger_apiKey");u.content="Actualités & Dernières Innovations IA :",u.widget="news",Mt(j||void 0)}else if(x.includes("parametre")||x.includes("reglage")||x.includes("configuration")||x.includes("setting"))u.content="Ouverture du panneau de configuration système :",u.widget="settings";else if(x.includes("[plugin:vite:")||x.includes("missing semicolon")||x.includes("syntaxerror")||x.includes("typescriptparsermixin")||x.includes("babel/parser")||x.includes("error")&&x.length>80){const j=q||"Projet_blog_8831";u.content=`🚨 Trace d'erreur détectée !

Lancement immédiat de l'Auto-Suture IA pour le projet "${j}" avec la stack trace fournie... 🩺`,Fe("suture",a)}else if(u.content="Traitement de votre demande via Tiger IA...",typeof window<"u"){const j=window.AndroidBridge;j&&j.openAIWithPrompt?j.openAIWithPrompt("https://chat.deepseek.com/",a):window.open("https://chat.deepseek.com/","_blank")}C(j=>[...j,u])},600),S("")},et=async s=>{var Ie;let a=Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s],l=null;const _=a.filter($=>$.name.endsWith(".zip"));let u=[];if(_.length>0){C($=>[...$,{id:Date.now().toString()+"_zip",role:"user",content:"📦 Extraction de l'archive ZIP en cours..."}]);try{const $=(await nn(async()=>{const{default:ue}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:ue}},[])).default;for(const ue of _){l=ue;const Ae=await new $().loadAsync(ue);for(const[oe,Ce]of Object.entries(Ae.files))if(!Ce.dir&&!oe.includes("__MACOSX")&&!((Ie=oe.split("/").pop())!=null&&Ie.startsWith("."))){const Ue=await Ce.async("blob"),mt=new File([Ue],oe.split("/").pop()||oe,{type:Ue.type});u.push(mt)}}}catch($){console.error("Erreur décompression ZIP:",$),alert("Système Kirov5 : Impossible de lire l'archive ZIP.")}for(const $ of _)try{const ue=await $.arrayBuffer(),rt=arrayBufferToBase64(ue);fetch("/api/fs/upload-zip",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:q||z||ce||"AUDIO",fileName:$.name,fileBase64:rt})}).catch(oe=>console.warn("[Upload ZIP Sync] Notice:",oe))}catch{}}a=[...a.filter($=>!$.name.endsWith(".zip")),...u];const x=a.filter($=>$.name.endsWith(".html")),j=a.filter($=>!$.name.endsWith(".html")&&($.name.endsWith(".png")||$.name.endsWith(".jpg")||$.name.endsWith(".jpeg")||$.name.endsWith(".md")||$.name.endsWith(".json")||$.name.endsWith(".txt")||$.name.endsWith(".tsx")||$.name.endsWith(".ts")||$.name.endsWith(".jsx")||$.name.endsWith(".js")||$.name.endsWith(".css")||$.name.endsWith(".svg")));if(x.length===0&&j.length===0){alert("Système Kirov5 : Format non supporté. Veuillez déposer au moins un .html, .md, .tsx, .ts, .png, ou un .zip.");return}let M=[...st];for(const $ of j){const ue=$.name.endsWith(".png")||$.name.endsWith(".jpg")||$.name.endsWith(".jpeg"),rt=await new Promise(oe=>{const Ce=new FileReader;Ce.onload=Ue=>{var mt;return oe((mt=Ue.target)==null?void 0:mt.result)},ue?Ce.readAsDataURL($):Ce.readAsText($)}),Ae={path:$.name,content:ue?`[Image jointe visuellement : ${$.name} (Binaire ignoré pour économie de tokens)]`:rt};M.some(oe=>oe.path===$.name)||M.push(Ae),C(oe=>[...oe,{id:Date.now().toString()+"_"+Math.random(),role:"user",content:`📎 Fichier de contexte ajouté au Trombone : ${$.name}`}])}if(x.length>0){const $=x[0],ue=await new Promise(oe=>{const Ce=new FileReader;Ce.onload=Ue=>{var mt;return oe((mt=Ue.target)==null?void 0:mt.result)},Ce.readAsText($)}),rt={path:$.name,content:ue};M.some(oe=>oe.path===$.name)||M.push(rt);const Ae={id:Date.now().toString()+"_main",role:"user",content:`📁 Fichier de design déposé : ${$.name}
Analyse de la structure UI et assemblage du contexte en cours...`};C(oe=>[...oe,Ae])}const Z=(Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s]).filter($=>$.name.endsWith(".zip"));for(const $ of Z)M.some(ue=>ue.path===$.name)||M.push({path:$.name,content:"ZIP_ARCHIVE_DUMMY"});kt(M);const J=q||ce||"Projet_Stitch",V=a.map($=>$.name).join(", ");console.log(`[TROMBONE] Fichiers déposés pour ${J}: ${V}`);const xe=x.length;if(xe>3){const $=localStorage.getItem("tiger_targetAi")||"deepseek",ue=l?"Projet_ZIP_"+l.name.replace(/[^a-zA-Z0-9]/g,"_").replace(".zip","")+"_"+Date.now().toString().slice(-4):"Projet_BATCH_"+Date.now().toString().slice(-4),rt=q||ue;q||je(rt),C(oe=>[...oe,{id:Date.now().toString()+"_batch",role:"assistant",content:`🤖 KIROV5 MULTI-BATCH DÉTECTÉ !

Hermes a détecté ${xe} pages HTML dans votre ZIP.
Découpage automatique en lots de 3 fichiers en cours...
Démarrage de l'automatisation séquentielle via DeepSeek !`,widget:"phases"}]),T(1);let Ae="";if(l)Ae=await new Promise(oe=>{const Ce=new FileReader;Ce.onload=Ue=>{var mt;return oe(((mt=Ue.target)==null?void 0:mt.result).split(",")[1])},Ce.readAsDataURL(l)});else{const oe=(await nn(async()=>{const{default:Ue}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:Ue}},[])).default,Ce=new oe;for(const Ue of a)Ce.file(Ue.name,Ue);Ae=await Ce.generateAsync({type:"base64"})}w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({zip_base64:Ae,zip_mode:!0,target_ai:$,target_project:rt,user_prompt:ce.trim()?`${ce} - ${bt}`:`Application multi-pages issue de ${(l==null?void 0:l.name)||"fichiers_locaux"}`})}).then(async oe=>{const Ce=await oe.json();Ce.success?(C(Ue=>[...Ue,{id:Date.now().toString()+"_hermes",role:"assistant",content:`✅ ${Ce.message}

📡 Lot 1/${Ce.total_batches} transmis à DeepSeek. Les lots suivants s'enchaîneront automatiquement !
Fichiers détectés : ${(Ce.files_detected||[]).join(", ")}`}]),window.open(gt($),"_blank")):alert("Erreur Hermes : "+(Ce.error||"Inconnue"))}).catch(oe=>{alert(`Erreur de connexion au Moteur Local : ${oe.message}`)});return}if(x.length>0){if(document.getElementById("creation-mode-container"))return;setTimeout(()=>{Ct(M)},1200)}},Ct=async(s=st)=>{Ne(!1);const a=s.find(J=>J.path.endsWith(".html"));if(!a){alert("Aucun fichier HTML trouvé dans le Trombone !");return}const l=ce.trim()?"Projet_"+ce.replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4):"Design_"+a.path.replace(".html","").replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4),_=q||l;if(!q){je(_);try{await w("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:_,file:"README.md",content:`# ${ce||_}

Initialisé par Tiger IA V0 (Design-First).`})})}catch(J){console.error("[IDE] Erreur création auto du dossier",J)}}const u=s.filter(J=>J.path!==a.path),x=a.content,j={id:(Date.now()+1).toString(),role:"assistant",content:`Mode "Design-First" activé. 🎨

Transmission du design au LLM avec les ${u.length} fichiers du Trombone intégrés.
Câblage en cours...`,widget:"phases"};T(5),C(J=>[...J,j]);let M=5;const Z=setInterval(()=>{M++,M>11?clearInterval(Z):T(M)},3e3);if(typeof window<"u"&&window.AndroidBridge&&window.AndroidBridge.showToast&&window.AndroidBridge.showToast("Mode Design-First Activé !"),typeof window<"u"){const J=localStorage.getItem("tiger_targetAi")||"deepseek";let V="";u.length>0&&(V=`
...
\`\`\`
`}));let xe="";(ce.trim()||We.trim()||bt.trim())&&(xe=`
--- MÉTADONNÉES DU PROJET ---
Nom : ${ce||"Non spécifié"}
Stack / Structure : ${We||"Non spécifiée"}
Description & Objectifs : ${bt||"Non spécifiés"}
-----------------------------
`);const Ie=`Voici le code HTML/CSS d'une interface générée par Stitch.${xe}
...
\`\`\`
`,$=window.AndroidBridge;if($&&$.openAIWithPrompt){const ue=gt(J);$.openAIWithPrompt(ue,Ie),$.showToast&&$.showToast("HTML injecté. Câblage sur "+J.toUpperCase()+" !")}else w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:J,prompt:Ie,auto_submit:!0,project_id:_,phase_num:5})}).then(ue=>console.log("[Bridge] Prompt HTML + Contexte envoyé avec succès, status:",ue.status)).catch(ue=>console.error("[Bridge] Erreur lors de l'envoi du prompt:",ue))}},Oe=(s,a=0)=>s?s.type==="directory"?e.jsxs("div",{className:"flex flex-col",children:[e.jsxs("div",{className:"flex items-center gap-2 px-2 py-1 hover:bg-white/5 cursor-pointer text-gray-300 text-sm",style:{paddingLeft:`${a*12+8}px`},children:[e.jsx("span",{className:"text-orange-400",children:"📁"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-dossier truncate font-bold",children:s.name})]}),s.children&&s.children.map(l=>Oe(l,a+1))]},s.path):e.jsxs("div",{className:`flex items-center justify-between px-2 py-1 hover:bg-white/10 cursor-pointer text-sm group ${fe===s.path?"bg-cyan/20 text-cyan border-l-2 border-cyan":"text-gray-400"}`,style:{paddingLeft:`${a*12+8}px`},onClick:()=>nt(s.path),children:[e.jsxs("div",{className:"flex items-center gap-2 truncate",children:[e.jsx("span",{className:"text-blue-400",children:"📄"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-fichier truncate",children:s.name})]}),e.jsxs("div",{className:"hidden group-hover:flex items-center gap-1",children:[e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-red-500/50 transition-colors",title:"Suture : Réparer ou débugger ce fichier",onClick:l=>{l.stopPropagation(),nt(s.path),setTimeout(()=>Fe("suture","Veuillez analyser ce fichier et corriger toutes les erreurs, fautes de frappe ou imports manquants."),100)},children:"🩺"}),e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-cyan/50 transition-colors",title:"Ajouter au Trombone",onClick:l=>{l.stopPropagation(),N(s.path)},children:"📎"})]})]},s.path):null,Fe=async(s,a)=>{if(!q){alert("⚠️ Veuillez sélectionner un projet actif dans l'explorateur.");return}const l=localStorage.getItem("tiger_targetAi")||"deepseek";let _="";s==="suture"?_=`🩺 SUTURE CHIRURGICALE — CORRECTION D'ERREUR DÉTECTÉE
...
`),fe&&Gt&&(_+=`--- Fichier Actif en Édition : ${fe} ---
\`\`\`
${Gt}
\`\`\`

`),st.length>0&&(_+=`--- Fichiers de contexte du Trombone ---

`,st.forEach(u=>{_+=`--- ${u.path} ---
\`\`\`
${u.content}
\`\`\`
...
{
  "files": [
    { "path": "${fe||"src/App.tsx"}", "content": "..." }
  ]
}
\`\`\``,C(u=>[...u,{id:Date.now().toString()+"_action",role:"assistant",content:`🩺 Suture & Correction déclenchée pour "${q}". Transmission de l'erreur à ${l.toUpperCase()} en cours...`}]);try{if(await w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:l,prompt:_,auto_submit:!0,project_id:q,phase_num:s})}),s==="suture"){w("http://localhost:5006/v1/suture/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:q,activeFile:fe||"",rawError:a||"",prompt:a||"Analyse et corrige les erreurs du projet."})}).then(async x=>{const j=await x.json();j.success&&j.status==="applied"?C(M=>{var Z,J;return[...M,{id:Date.now().toString()+"_suture_ok",role:"assistant",content:`✅ Suture V2 réussie !

🔧 ${j.repairId}
🚀 Statut : **Patch appliqué en production**
📁 Fichiers corrigés : ${JSON.stringify(((J=(Z=j.patchReport)==null?void 0:Z.files)==null?void 0:J.map(V=>V.path))||[])}`}]}):j.success?C(M=>{var Z;return[...M,{id:Date.now().toString()+"_suture_partial",role:"assistant",content:`⚠️ Suture V2 : correctif généré mais non promu.
...
`:We.includes("Canvas 2D")?M+=`⚠️ CONSTRUCTEUR : Ce projet DOIT IMPÉRATIVEMENT être un jeu Web 2D généré en **React + Vite + TypeScript (TSX) + Canvas 2D**.
