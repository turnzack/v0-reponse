const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-v200-BM1NjLya.js","assets/design-v200-DNEogQpL.js","assets/design-v200-C8HtVSRB.css"])))=>i.map(i=>d[i]);
import{r as a,a as At,j as e,s as w,i as us,R as ps}from"./design-v200-DNEogQpL.js";const ms="modulepreload",gs=function(t){return"/"+t},Nn={},Xt=function(n,r,o){let d=Promise.resolve();if(r&&r.length>0){let m=function(v){return Promise.all(v.map(h=>Promise.resolve(h).then(q=>({status:"fulfilled",value:q}),q=>({status:"rejected",reason:q}))))};document.getElementsByTagName("link");const u=document.querySelector("meta[property=csp-nonce]"),_=(u==null?void 0:u.nonce)||(u==null?void 0:u.getAttribute("nonce"));d=m(r.map(v=>{if(v=gs(v),v in Nn)return;Nn[v]=!0;const h=v.endsWith(".css"),q=h?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${v}"]${q}`))return;const N=document.createElement("link");if(N.rel=h?"stylesheet":ms,h||(N.as="script"),N.crossOrigin="",N.href=v,_&&N.setAttribute("nonce",_),document.head.appendChild(N),h)return new Promise(($,I)=>{N.addEventListener("load",$),N.addEventListener("error",()=>I(new Error(`Unable to preload CSS for ${v}`)))})}))}function p(m){const u=new Event("vite:preloadError",{cancelable:!0});if(u.payload=m,window.dispatchEvent(u),!u.defaultPrevented)throw m}return d.then(m=>{for(const u of m||[])u.status==="rejected"&&p(u.reason);return n().catch(p)})},hs=`# undefined
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const In=ae("TriangleAlert",[["path",{d:"m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3",key:"wmoenq"}],["path",{d:"M12 9v4",key:"juzpu7"}],["path",{d:"M12 17h.01",key:"p32p05"}]]);/**
 * @license lucide-react v0.378.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
...
3. Création du projet et câblage avec Stitch.`}];try{if(typeof window<"u"&&sessionStorage.getItem("deepseek_402")){at(c),ge(!1);return}const l=s||localStorage.getItem("tiger_apiKey");if(!l||!l.startsWith("sk-")||l.length<20||typeof window<"u"&&localStorage.getItem("deepseek_solde_epuise")==="true"){at(c),ge(!1);return}const k=new AbortController,f=setTimeout(()=>k.abort(),4e3),x=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${l}`},signal:k.signal,body:JSON.stringify({model:"deepseek-chat",messages:[{role:"system",content:"Tu es le fil d'actualités technologiques de l'OS Souverain. Réponds UNIQUEMENT en JSON avec une clé 'news' contenant 3 articles récents au format [{id, tag, title, desc, content}]."},{role:"user",content:"Donne 3 actualités récentes sur l'IA et le développement web."}],response_format:{type:"json_object"}})});if(clearTimeout(f),x.ok){const j=await x.json(),D=JSON.parse(j.choices[0].message.content);D.news&&Array.isArray(D.news)&&D.news.length>0?at(D.news):at(c)}else x.status===402&&typeof window<"u"&&(sessionStorage.setItem("deepseek_402","true"),localStorage.setItem("deepseek_solde_epuise","true")),at(c)}catch(l){console.warn("[Actualités] Erreur ou Timeout API DeepSeek, chargement fallback:",l),at(c)}finally{ge(!1)}},[]);a.useEffect(()=>{zt()},[zt]);const[ot,yt]=a.useState(!1),[Oe,Rt]=a.useState(()=>localStorage.getItem("tiger_active_theme")||"fold"),[Ce,Ve]=a.useState([]),[Se,Nt]=a.useState("");a.useEffect(()=>{const s=localStorage.getItem("tiger_saved_themes");if(s)try{let c=JSON.parse(s);if(!Array.isArray(c))throw new Error("Not an array");const l="linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)";c=c.map(k=>k.name==="fold"?{...k,colors:{...k.colors,"bg-app":l}}:k),Ve(c)}catch{Ve([])}else{const c={name:"fold",colors:{"icon-settings":"linear-gradient(135deg, #c87058, #934a36)","icon-ai":"linear-gradient(135deg, #9d508e, #622e5a)","icon-projects":"linear-gradient(135deg, #d38b5d, #a26038)","icon-packs":"linear-gradient(135deg, #445499, #252e66)","icon-news":"linear-gradient(135deg, #389eb2, #1f6475)","bg-app":"linear-gradient(135deg, #4c1d95 0%, #1e1b4b 35%, #0f172a 70%, #06b6d4 100%)"}};Ve([c]),localStorage.setItem("tiger_saved_themes",JSON.stringify([c]))}},[]),a.useEffect(()=>{var s;if(localStorage.setItem("tiger_active_theme",Oe),Oe!=="random"){const c=((s=Ce.find(l=>l.name===Oe))==null?void 0:s.colors["bg-app"])||"linear-gradient(135deg, #1e1b4b 0%, #0f172a 50%, #000000 100%)";document.body.style.background=c,document.body.style.backgroundAttachment="fixed"}else document.body.style.background=""},[Oe,Ce]);const an=()=>{if(!Se.trim())return alert("Nom invalide");if(Ce.length>=10)return alert("Limite atteinte ! Vous ne pouvez sauvegarder que 10 thèmes maximum. Veuillez en supprimer un pour continuer.");const s={"icon-settings":o("icon-settings",.8),"icon-ai":o("icon-ai",.8),"icon-projects":o("icon-projects",.8),"icon-packs":o("icon-packs",.8),"icon-news":o("icon-news",.8),"bg-app":o("bg-app",.9)},c={name:Se.trim(),colors:s},l=[...Ce,c];Ve(l),localStorage.setItem("tiger_saved_themes",JSON.stringify(l)),Rt(c.name),Nt("")},Be=(s,c)=>{c.stopPropagation();const l=Ce.filter(k=>k.name!==s);Ve(l),localStorage.setItem("tiger_saved_themes",JSON.stringify(l)),Oe===s&&Rt("random")},Ht=s=>{H(c=>c.includes(s)?c.filter(l=>l!==s):[...c,s])},[Fe,Dt]=a.useState(()=>{if(typeof window<"u"){const s=sessionStorage.getItem("tiger_selectedStartPhase");return s?Number(s):0}return 0});a.useEffect(()=>{typeof window<"u"&&sessionStorage.setItem("tiger_selectedStartPhase",String(Fe))},[Fe]);const[dt,Kt]=a.useState(!0),[Ze,_t]=a.useState(!0),[U,we]=a.useState(null),[Ct,fn]=a.useState(null),[Ie,$t]=a.useState(null),[de,Ye]=a.useState(null),[wt,ut]=a.useState(""),[ze,Mt]=a.useState([]);a.useEffect(()=>{w("http://localhost:5006/api/projects").then(s=>s?s.json():null).then(s=>{if(s&&s.success&&Array.isArray(s.projects))ie(s.projects.map(c=>({name:c,desc:"Projet local",bg:""})));else{const c=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:c?{Authorization:`Bearer ${c}`}:{}}).then(l=>l?l.json():null).then(l=>{l&&l.projects&&ie(l.projects.map(k=>({name:k.project_id||k.title,desc:"SaaS Cloud",bg:""})))}).catch(()=>{})}}).catch(()=>{})},[]);const[Ge,je]=a.useState(null),[It,pt]=a.useState(""),[Ee,on]=a.useState(!1),[et,He]=a.useState(!1),i=a.useRef(null);a.useEffect(()=>{if(i.current=null,U==="v0-guest"||U==="V0-Guest")He(!0),je("http://localhost:3007"),pt("http://localhost:3007"),i.current="http://localhost:3007";else if(U){He(!0);const s=Ie&&JSON.stringify(Ie).includes("next.config"),c=typeof window<"u"&&window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1",l=s?"http://localhost:3000":i.current||(c?`http://${window.location.hostname}:5173`:"http://localhost:5173");je(l),pt(l),i.current=l}else He(!1),je(null)},[U,Ie]),a.useEffect(()=>{const s=c=>{var l,k,f,x;if(((l=c.data)==null?void 0:l.type)==="CHANGE_PREVIEW_URL"){const j=c.data.route,D=j==="/"?"":`#${j}`;je(se=>{const Z=se?se.split("#")[0]:"http://127.0.0.1:5175/";return`${Z.endsWith("/")?Z:`${Z}/`}${D}`})}if(((k=c.data)==null?void 0:k.type)==="DESIGN_ELEMENT_CLICKED"||((f=c.data)==null?void 0:f.type)==="DESIGN_ELEMENT_DRAGGED"||((x=c.data)==null?void 0:x.type)==="DESIGN_ELEMENT_RESIZED"){const j=document.querySelector('iframe[title="Studio Admin Design"]');j&&j.contentWindow&&j.contentWindow.postMessage(c.data,"*")}};return window.addEventListener("message",s),()=>window.removeEventListener("message",s)},[]),a.useEffect(()=>{U&&w(`http://localhost:5006/api/fs/tree?project=${U}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&$t(s.tree)}).catch(()=>{})},[U]),a.useEffect(()=>{U&&de&&w(`http://localhost:5006/api/fs/read?project=${U}&file=${encodeURIComponent(de)}`).then(s=>s?s.json():null).then(s=>{s&&s.success&&ut(s.content)}).catch(()=>{})},[U,de]);const g=async s=>{if(s===void 0||!U||!de)return;if(ut(s),Ft.isNativePlatform()||!!window.AndroidBridge)try{await Vn.writeFile({path:`v0-moteur-mobile/projetv0/${U}/${de}`,data:s,directory:pn.Documents,encoding:wn.UTF8,recursive:!0})}catch(l){console.error("Erreur de sauvegarde mobile:",l)}else w("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:U,file:de,content:s})}).catch(()=>{})},b=s=>{U&&w(`http://localhost:5006/api/fs/read?project=${U}&file=${encodeURIComponent(s)}`).then(c=>c?c.json():null).then(c=>{c&&c.success&&Mt(l=>l.find(k=>k.path===s)?l:[...l,{path:s,content:c.content}])}).catch(()=>{})};a.useEffect(()=>{(async()=>{const c=["bg-gradient-to-br from-[#bf6969]/80 to-[#c27042]/90 backdrop-blur-md","bg-gradient-to-br from-[#a387b9]/80 to-[#aa6b73]/90 backdrop-blur-md","bg-gradient-to-br from-[#e4a37f]/80 to-[#bf6969]/90 backdrop-blur-md","bg-gradient-to-br from-[#aa6b73]/80 to-[#c27042]/90 backdrop-blur-md"];if(Ft.isNativePlatform()||!!window.AndroidBridge)try{const f=(await Vn.readdir({path:"v0-moteur-mobile/projetv0",directory:pn.Documents})).files.map(x=>x.name||x.toString());ie(f.map((x,j)=>({name:x,desc:"Mémoire Téléphone",bg:c[j%c.length]})))}catch{console.log("Aucun projet mobile trouvé ou dossier inexistant.")}else w("http://localhost:5006/api/projects-v2").then(k=>k?k.json():null).then(k=>{if(k&&k.success&&k.projects)ie(k.projects.map((f,x)=>({name:f.name,desc:"Environnement Local",bg:c[x%c.length],installed:f.installed})));else{const f=localStorage.getItem("kirov5_jwt_token")||"";fetch("/api/projects",{headers:f?{Authorization:`Bearer ${f}`}:{}}).then(x=>x?x.json():null).then(x=>{x&&x.projects&&ie(x.projects.map((j,D)=>({name:j.project_id||j.title,desc:"SaaS Cloud (Neon DB)",bg:c[D%c.length]})))}).catch(()=>{})}}).catch(()=>{})})()},[]),a.useEffect(()=>{G(!0)},[]);const[R,Q]=a.useState(!0);a.useEffect(()=>{const s=()=>Q(!0);window.addEventListener("open-mouchard",s);const c=l=>{if(l.data&&l.data.type==="TIGER_CAPTURE"){const k=l.data.data;if(k){let f="index.html";(k.includes("import React")||k.includes("export default"))&&(f="App.tsx"),k.includes("tailwindcss")&&(f="index.css"),Ye(f),g(k),$t(x=>{var se;const j={name:f,path:f,type:"file"};return x?((se=x.children)==null?void 0:se.find(Z=>Z.name===f))?x:{...x,children:[...x.children||[],j]}:{name:"Mobile_Storage",type:"directory",children:[j]}})}}};return window.addEventListener("message",c),()=>{window.removeEventListener("open-mouchard",s),window.removeEventListener("message",c)}},[U,de]),a.useEffect(()=>{if(!E)return;const s=()=>{w("http://localhost:5006/api/bridge/queue").then(l=>l?l.json():null).then(l=>{l&&l.success&&(setBridgeQueueData({current:l.current||{},queue:l.queue||[]}),(l.queue&&l.queue.length>0||l.current&&Object.keys(l.current).length>0)&&setIsPipelineRunning(!0))}).catch(()=>{}),w("http://localhost:5006/api/bridge/logs").then(l=>l?l.json():null).then(l=>{var k,f;if(l&&l.success&&l.logs){he(l.logs);const x=[...l.logs].reverse().find(D=>D.includes("URL_PREVIEW="));if(x){let D=(k=x.split("URL_PREVIEW=")[1])==null?void 0:k.trim();D&&typeof window<"u"&&window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1"&&(D=D.replace(/localhost|127\.0\.0\.1/,window.location.hostname)),D&&D!==i.current&&(i.current=D,je(D),pt(D),He(!0))}const j=[...l.logs].reverse().find(D=>D.includes("PIPELINE COMPLET TERMINÉ avec succès pour "));if(j){const D=(f=j.split("PIPELINE COMPLET TERMINÉ avec succès pour ")[1])==null?void 0:f.replace(/[.\s]+$/,"");D&&U!==D&&(we(D),He(!0))}}}).catch(()=>{}),w("http://localhost:5006/api/bridge/autonomous-status").then(l=>l?l.json():null).then(l=>{if(l&&l.success&&(l.data||l.status)){const f=(l.data||l.status).state;f==="diagnosing"||f==="repair_required"||f==="error"?S("error"):["repair_planned","patching_staging","validating","browser_verifying","repairing","working"].includes(f)||["starting_server","launch_requested","restarting"].includes(f)?S("working"):f==="ready"?S("clean"):U&&w(`http://localhost:5006/api/suture/status?project=${U}`).then(x=>x?x.json():null).then(x=>{x&&x.success&&x.status&&S(x.status)}).catch(()=>{})}}).catch(()=>{})};s();const c=setInterval(s,2e3);return()=>clearInterval(c)},[E,U]);const P=s=>{const c=typeof s=="string"?s:v;if(!c.trim())return;const l={id:Date.now().toString(),role:"user",content:c};_(f=>[...f,l]);const k=c.toLowerCase();setTimeout(()=>{let f={id:(Date.now()+1).toString(),role:"assistant",content:""};const x=k.normalize("NFD").replace(/[\u0300-\u036f]/g,"");if(x==="mes projets"||x.includes("liste des projet")||x==="projet"||x==="projets")f.content="Voici la liste de vos projets récents :",f.widget="projects";else if((x.includes("stitch")||x.includes("deepseek")||x.includes("design")||x.includes("logique"))&&(x.includes("modifi")||x.includes("ajoute")||x.includes("change")||x.includes("mise a jour")||x.includes("evolue")||x.includes("reprends")||x.includes("continue"))){const j=c.match(/\[(.*?)\]/),D=j?j[1]:null,se=x.includes("deepseek")?"deepseek":"stitch";f.content=`🔄 MODE ÉVOLUTION ACTIVÉ (${se.toUpperCase()}) 🔄

J'injecte vos nouvelles directives directement dans votre interface... ${D?`
🔍 Recherche et AutoSwitch vers le projet : "${D}"`:"Reprise du travail en cours."}`,f.widget=null,w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:se,target_project:D,prompt:"Mise à jour (Reprise de projet) : "+c.replace(/\[.*?\]/,"").trim(),auto_submit:!0})}).catch(Z=>console.log("Erreur de connexion au Bridge local pour l'injection",Z))}else if(x.includes("cree")||x.includes("lance")||x.includes("projet")||x.includes("generation")){f.content=`🚀 DÉMARRAGE PARALLÈLE KIROV5 🚀
...
Les intelligences artificielles sont informées et en attente. Une fois le design terminé, glissez l'HTML ici pour lancer le câblage final en phase 5.`,f.widget="phases",z(1);const j=U||(me.trim()?me.trim().replace(/[^a-zA-Z0-9_-]/g,"_"):"Projet_"+c.substring(0,15).replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4));if(we(j),typeof window<"u"&&localStorage.setItem("tiger_lastGeneratedProject",j),typeof window<"u"){const Z=localStorage.getItem("tiger_targetAi")||"deepseek",Y=localStorage.getItem("tiger_targetUiAi")||"stitch",Te=(O,le=!1)=>st(O,le),De=window.AndroidBridge;if(De&&De.openAIWithPrompt)De.openAIWithPrompt(Te(Y,!0),"Génère l'interface UI/UX complète et moderne pour ce projet : "+c),De.showToast&&De.showToast("Stitch s'ouvre. Générez le HTML, puis utilisez le Trombone.");else{const O=T&&T.length>0?`

[PACKS PRD ARCHITECTURE SELECTIONNES (${T.length})]
`+T.map(ve=>{const re=Lt.find(xe=>xe.id===ve);return`• ${re?re.name:ve} (#${ve})`}).join(`
`):"",le="Génère l'interface UI/UX complète et moderne pour ce projet : "+c+O;try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(le)}catch{}const $e=()=>w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Y,prompt:le,auto_submit:!0,project_id:j,phase_num:1,packs:T})});Fe!==200&&(Ze?window.open(Te(Y,!0),"kirov5_ai_target"):window.open(Te(Y,!0),"_blank"),$e().then(()=>{console.log("Prompt UI envoyé avec succès au Bridge pour Stitch !")}).catch(ve=>console.error("Erreur sendUiPrompt",ve))),setTimeout(()=>{const ve=T&&T.length>0?c+O:"L'interface UI/UX est actuellement en cours de génération. Prépare la structure backend et les états React pour un projet complexe : "+c+". Reste en attente, je te fournirai le fichier HTML pour le câblage final.";try{typeof navigator<"u"&&navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(ve)}catch{}window.open(Te(Z),"_blank"),w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:ve,auto_submit:!0,project_id:j,phase_num:1,packs:T})}).catch(re=>console.log("Erreur Bridge Logic Prompt:",re)),T&&T.length>0?w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,user_prompt:ve,packs:T,target_project:j})}).then(()=>{console.log("Méga-Prompt Trombone envoyé !")}).catch(re=>{console.log("Erreur Trombone Bridge:",re)}):w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:ve,auto_submit:!0,project_id:j,phase_num:1})}).then(()=>{console.log("Prompt Logique envoyé au Bridge")}).catch(()=>{})},800),console.log("Les IA ont été ouvertes. Les prompts ont été envoyés au Bridge local pour injection via l'extension.")}}z(1);let D=1;const se=setInterval(()=>{D++,D>11?clearInterval(se):z(D)},1500)}else if(x.includes("youtube")||x.includes("video"))f.content="Voici les résultats YouTube pour votre recherche :",f.widget="youtube";else if(x.includes("actualite")||x.includes("ia")||x.includes("news")||x.includes("recherche")){const j=localStorage.getItem("tiger_apiKey");f.content="Actualités & Dernières Innovations IA :",f.widget="news",zt(j||void 0)}else if(x.includes("parametre")||x.includes("reglage")||x.includes("configuration")||x.includes("setting"))f.content="Ouverture du panneau de configuration système :",f.widget="settings";else if(x.includes("[plugin:vite:")||x.includes("missing semicolon")||x.includes("syntaxerror")||x.includes("typescriptparsermixin")||x.includes("babel/parser")||x.includes("error")&&x.length>80){const j=U||"Projet_blog_8831";f.content=`🚨 Trace d'erreur détectée !

Lancement immédiat de l'Auto-Suture IA pour le projet "${j}" avec la stack trace fournie... 🩺`,Pe("suture",c)}else if(f.content="Traitement de votre demande via Tiger IA...",typeof window<"u"){const j=window.AndroidBridge;j&&j.openAIWithPrompt?j.openAIWithPrompt("https://chat.deepseek.com/",c):window.open("https://chat.deepseek.com/","_blank")}_(j=>[...j,f])},600),h("")},ue=async s=>{var De;let c=Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s],l=null;const k=c.filter(O=>O.name.endsWith(".zip"));let f=[];if(k.length>0){_(O=>[...O,{id:Date.now().toString()+"_zip",role:"user",content:"📦 Extraction de l'archive ZIP en cours..."}]);try{const O=(await Xt(async()=>{const{default:le}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:le}},[])).default;for(const le of k){l=le;const ve=await new O().loadAsync(le);for(const[re,xe]of Object.entries(ve.files))if(!xe.dir&&!re.includes("__MACOSX")&&!((De=re.split("/").pop())!=null&&De.startsWith("."))){const ke=await xe.async("blob"),nt=new File([ke],re.split("/").pop()||re,{type:ke.type});f.push(nt)}}}catch(O){console.error("Erreur décompression ZIP:",O),alert("Système Kirov5 : Impossible de lire l'archive ZIP.")}for(const O of k)try{const le=await O.arrayBuffer(),$e=arrayBufferToBase64(le);fetch("/api/fs/upload-zip",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:U||L||me||"AUDIO",fileName:O.name,fileBase64:$e})}).catch(re=>console.warn("[Upload ZIP Sync] Notice:",re))}catch{}}c=[...c.filter(O=>!O.name.endsWith(".zip")),...f];const x=c.filter(O=>O.name.endsWith(".html")),j=c.filter(O=>!O.name.endsWith(".html")&&(O.name.endsWith(".png")||O.name.endsWith(".jpg")||O.name.endsWith(".jpeg")||O.name.endsWith(".md")||O.name.endsWith(".json")||O.name.endsWith(".txt")||O.name.endsWith(".tsx")||O.name.endsWith(".ts")||O.name.endsWith(".jsx")||O.name.endsWith(".js")||O.name.endsWith(".css")||O.name.endsWith(".svg")));if(x.length===0&&j.length===0){alert("Système Kirov5 : Format non supporté. Veuillez déposer au moins un .html, .md, .tsx, .ts, .png, ou un .zip.");return}let D=[...ze];for(const O of j){const le=O.name.endsWith(".png")||O.name.endsWith(".jpg")||O.name.endsWith(".jpeg"),$e=await new Promise(re=>{const xe=new FileReader;xe.onload=ke=>{var nt;return re((nt=ke.target)==null?void 0:nt.result)},le?xe.readAsDataURL(O):xe.readAsText(O)}),ve={path:O.name,content:le?`[Image jointe visuellement : ${O.name} (Binaire ignoré pour économie de tokens)]`:$e};D.some(re=>re.path===O.name)||D.push(ve),_(re=>[...re,{id:Date.now().toString()+"_"+Math.random(),role:"user",content:`📎 Fichier de contexte ajouté au Trombone : ${O.name}`}])}if(x.length>0){const O=x[0],le=await new Promise(re=>{const xe=new FileReader;xe.onload=ke=>{var nt;return re((nt=ke.target)==null?void 0:nt.result)},xe.readAsText(O)}),$e={path:O.name,content:le};D.some(re=>re.path===O.name)||D.push($e);const ve={id:Date.now().toString()+"_main",role:"user",content:`📁 Fichier de design déposé : ${O.name}
Analyse de la structure UI et assemblage du contexte en cours...`};_(re=>[...re,ve])}const se=(Array.isArray(s)?s:s instanceof FileList?Array.from(s):[s]).filter(O=>O.name.endsWith(".zip"));for(const O of se)D.some(le=>le.path===O.name)||D.push({path:O.name,content:"ZIP_ARCHIVE_DUMMY"});Mt(D);const Z=U||me||"Projet_Stitch",Y=c.map(O=>O.name).join(", ");console.log(`[TROMBONE] Fichiers déposés pour ${Z}: ${Y}`);const Te=x.length;if(Te>3){const O=localStorage.getItem("tiger_targetAi")||"deepseek",le=l?"Projet_ZIP_"+l.name.replace(/[^a-zA-Z0-9]/g,"_").replace(".zip","")+"_"+Date.now().toString().slice(-4):"Projet_BATCH_"+Date.now().toString().slice(-4),$e=U||le;U||we($e),_(re=>[...re,{id:Date.now().toString()+"_batch",role:"assistant",content:`🤖 KIROV5 MULTI-BATCH DÉTECTÉ !

Hermes a détecté ${Te} pages HTML dans votre ZIP.
Découpage automatique en lots de 3 fichiers en cours...
Démarrage de l'automatisation séquentielle via DeepSeek !`,widget:"phases"}]),z(1);let ve="";if(l)ve=await new Promise(re=>{const xe=new FileReader;xe.onload=ke=>{var nt;return re(((nt=ke.target)==null?void 0:nt.result).split(",")[1])},xe.readAsDataURL(l)});else{const re=(await Xt(async()=>{const{default:ke}=await import("https://cdn.jsdelivr.net/npm/jszip@3.10.1/+esm");return{default:ke}},[])).default,xe=new re;for(const ke of c)xe.file(ke.name,ke);ve=await xe.generateAsync({type:"base64"})}w("/api/bridge/trombone",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({zip_base64:ve,zip_mode:!0,target_ai:O,target_project:$e,user_prompt:me.trim()?`${me} - ${lt}`:`Application multi-pages issue de ${(l==null?void 0:l.name)||"fichiers_locaux"}`})}).then(async re=>{const xe=await re.json();xe.success?(_(ke=>[...ke,{id:Date.now().toString()+"_hermes",role:"assistant",content:`✅ ${xe.message}

📡 Lot 1/${xe.total_batches} transmis à DeepSeek. Les lots suivants s'enchaîneront automatiquement !
Fichiers détectés : ${(xe.files_detected||[]).join(", ")}`}]),window.open(st(O),"_blank")):alert("Erreur Hermes : "+(xe.error||"Inconnue"))}).catch(re=>{alert(`Erreur de connexion au Moteur Local : ${re.message}`)});return}if(x.length>0){if(document.getElementById("creation-mode-container"))return;setTimeout(()=>{Ae(D)},1200)}},Ae=async(s=ze)=>{rt(!1);const c=s.find(Z=>Z.path.endsWith(".html"));if(!c){alert("Aucun fichier HTML trouvé dans le Trombone !");return}const l=me.trim()?"Projet_"+me.replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4):"Design_"+c.path.replace(".html","").replace(/[^a-zA-Z0-9]/g,"_")+"_"+Date.now().toString().slice(-4),k=U||l;if(!U){we(k);try{await w("http://localhost:5006/api/fs/write",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project:k,file:"README.md",content:`# ${me||k}

Initialisé par Tiger IA V0 (Design-First).`})})}catch(Z){console.error("[IDE] Erreur création auto du dossier",Z)}}const f=s.filter(Z=>Z.path!==c.path),x=c.content,j={id:(Date.now()+1).toString(),role:"assistant",content:`Mode "Design-First" activé. 🎨

Transmission du design au LLM avec les ${f.length} fichiers du Trombone intégrés.
Câblage en cours...`,widget:"phases"};z(5),_(Z=>[...Z,j]);let D=5;const se=setInterval(()=>{D++,D>11?clearInterval(se):z(D)},3e3);if(typeof window<"u"&&window.AndroidBridge&&window.AndroidBridge.showToast&&window.AndroidBridge.showToast("Mode Design-First Activé !"),typeof window<"u"){const Z=localStorage.getItem("tiger_targetAi")||"deepseek";let Y="";f.length>0&&(Y=`
...
\`\`\`
`,O=window.AndroidBridge;if(O&&O.openAIWithPrompt){const le=st(Z);O.openAIWithPrompt(le,De),O.showToast&&O.showToast("HTML injecté. Câblage sur "+Z.toUpperCase()+" !")}else w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:Z,prompt:De,auto_submit:!0,project_id:k,phase_num:5})}).then(le=>console.log("[Bridge] Prompt HTML + Contexte envoyé avec succès, status:",le.status)).catch(le=>console.error("[Bridge] Erreur lors de l'envoi du prompt:",le))}},tt=(s,c=0)=>s?s.type==="directory"?e.jsxs("div",{className:"flex flex-col",children:[e.jsxs("div",{className:"flex items-center gap-2 px-2 py-1 hover:bg-white/5 cursor-pointer text-gray-300 text-sm",style:{paddingLeft:`${c*12+8}px`},children:[e.jsx("span",{className:"text-orange-400",children:"📁"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-dossier truncate font-bold",children:s.name})]}),s.children&&s.children.map(l=>tt(l,c+1))]},s.path):e.jsxs("div",{className:`flex items-center justify-between px-2 py-1 hover:bg-white/10 cursor-pointer text-sm group ${de===s.path?"bg-cyan/20 text-cyan border-l-2 border-cyan":"text-gray-400"}`,style:{paddingLeft:`${c*12+8}px`},onClick:()=>Ye(s.path),children:[e.jsxs("div",{className:"flex items-center gap-2 truncate",children:[e.jsx("span",{className:"text-blue-400",children:"📄"}),e.jsx("span",{className:"design-explorateur-texte design-explorateur-fichier truncate",children:s.name})]}),e.jsxs("div",{className:"hidden group-hover:flex items-center gap-1",children:[e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-red-500/50 transition-colors",title:"Suture : Réparer ou débugger ce fichier",onClick:l=>{l.stopPropagation(),Ye(s.path),setTimeout(()=>Pe("suture","Veuillez analyser ce fichier et corriger toutes les erreurs, fautes de frappe ou imports manquants."),100)},children:"🩺"}),e.jsx("button",{className:"text-xs text-white bg-white/20 rounded px-1 hover:bg-cyan/50 transition-colors",title:"Ajouter au Trombone",onClick:l=>{l.stopPropagation(),b(s.path)},children:"📎"})]})]},s.path):null,Pe=async(s,c)=>{if(!U){alert("⚠️ Veuillez sélectionner un projet actif dans l'explorateur.");return}const l=localStorage.getItem("tiger_targetAi")||"deepseek";let k="";s==="suture"?k=`🩺 SUTURE CHIRURGICALE — CORRECTION D'ERREUR DÉTECTÉE
...
`),de&&wt&&(k+=`--- Fichier Actif en Édition : ${de} ---
\`\`\`
${wt}
\`\`\`
...
\`\`\``,_(f=>[...f,{id:Date.now().toString()+"_action",role:"assistant",content:`🩺 Suture & Correction déclenchée pour "${U}". Transmission de l'erreur à ${l.toUpperCase()} en cours...`}]);try{if(await w("http://localhost:5006/bridge/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target_ai:l,prompt:k,auto_submit:!0,project_id:U,phase_num:s})}),s==="suture"){w("http://localhost:5006/v1/suture/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({project_id:U,activeFile:de||"",rawError:c||"",prompt:c||"Analyse et corrige les erreurs du projet."})}).then(async x=>{const j=await x.json();j.success&&j.status==="applied"?_(D=>{var se,Z;return[...D,{id:Date.now().toString()+"_suture_ok",role:"assistant",content:`✅ Suture V2 réussie !

🔧 ${j.repairId}
🚀 Statut : **Patch appliqué en production**
📁 Fichiers corrigés : ${JSON.stringify(((Z=(se=j.patchReport)==null?void 0:se.files)==null?void 0:Z.map(Y=>Y.path))||[])}`}]}):j.success?_(D=>{var se;return[...D,{id:Date.now().toString()+"_suture_partial",role:"assistant",content:`⚠️ Suture V2 : correctif généré mais non promu.
Statut : ${((se=j.validationReport)==null?void 0:se.code)||j.status}
